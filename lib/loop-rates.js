...
function mapFluidVaultNft(wallet, chain, nft) {
  ...
  return {
    id: `fluid-vault:${wallet}:${chain.chainId}:${nft.id || nft.nftId || vault.id}`,
    protocol: 'Fluid',
    ...
    supplied: supplyUsd > 0.01 ? [{ symbol: supplyToken?.symbol || 'Collateral', value: supplyUsd, apy: supplyApy }] : [],
    borrowed: borrowUsd > 0.01 ? [{ symbol: borrowToken?.symbol || 'Debt', value: borrowUsd, apy: borrowApy }] : [],
    totalSupplied: supplyUsd,
    totalBorrowed: borrowUsd,
...
function enrichPositionWithDefillamaYield(position, index) {
  ...
  for (const leg of position.supplied || []) {
    if (num(leg.apy, 0) > 0.01) continue;
    ...
    leg.apy = dlApy;
    position.suppliedYieldUsd = num(position.suppliedYieldUsd, 0) + num(leg.value) * dlApy;
    touched = true;
  }
...
function merklEarnedUsdFromToken(earnedToken) {
  const amount = earnedToken?.amount;
  const token = earnedToken?.token || {};
  const units = morphoRawUnits(amount, token.decimals);
  const px = num(token.price, 0);
  return units > 0 && px > 0 ? units * px : 0;
}
...
function buildMerklEarnedUsdMap(entries, positions) {
  ...
  for (const { items } of entries || []) {
    for (const item of items || []) {
      ...
      if (!opp) continue;
      let earnedUsd = 0;
      for (const earnedToken of item.earnedTokens || []) {
        earnedUsd += merklEarnedUsdFromToken(earnedToken);
      }
      if (earnedUsd <= 0.01) continue;

      const matches = (positions || []).filter(p => positionMatchesMerklOpportunity(p, opp));
      if (!matches.length) continue;
      const weightTotal = matches.reduce((sum, p) => sum + (Math.abs(num(p.netValue)) || num(p.totalSupplied) || 1), 0) || matches.length;
      for (const position of matches) {
        const weight = Math.abs(num(position.netValue)) || num(position.totalSupplied) || 1;
        const share = weight / weightTotal;
        earnedByPosition[position.id] = num(earnedByPosition[position.id]) + earnedUsd * share;
      }
    }
  }
  return earnedByPosition;
}

function applyMerklRewardsToPositions(positions, earnedByPosition) {
  for (const position of positions || []) {
    const merklRewardsUsd = num(earnedByPosition?.[position.id]);
    if (merklRewardsUsd <= 0.01) continue;
...
async function fetchLoopRates({ wallets }) {
  ...
  const fluidChainIds = Array.isArray(fluidPools)
    ...
    : [1, 42161, 8453, 137];
  const fluid = await fetchFluidOfficial(cleanWallets, fluidChainIds);
  errors.push(...fluid.errors);

  const defillamaYield = await fetchDefillamaYieldApyIndex();
  if (defillamaYield.error) errors.push({ provider: 'defillama', message: defillamaYield.error });

  const merkl = await fetchMerklActiveOpportunities(cleanWallets);
  errors.push(...merkl.errors);

  let positions = [...aavePositions, ...morphoPositions, ...fluid.positions]
    .filter(p => p.totalBorrowed > 0.01 || p.vaultOnly)
    .map(pos => enrichPositionWithDefillamaYield(pos, defillamaYield))
    .map(pos => enrichPositionWithMerkl(pos, merkl.index));

  const merklEarnedByPosition = buildMerklEarnedUsdMap(merkl.entries, positions);
  positions = applyMerklRewardsToPositions(positions, merklEarnedByPosition)
  ...
  return {
    ...
    coverage: {
      ...
      merklRewardSource: 'merkl-active-opportunities',
