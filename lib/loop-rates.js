  ...
  try {
    ...
    for (const pool of json.data || []) {
      ...
      for (const addr of pool.underlyingTokens || []) {
        ...
        if (!prev || score > prev.score) {
          byAddress.set(key, { apy, poolId: pool.pool, score, project: pool.project, symbol: pool.symbol });
        }
      }
    }
...
function enrichPositionWithDefillamaYield(position, index, chart7dCache = null) {
  ...
  for (const leg of position.supplied || []) {
    ...
    leg.defillamaApySpot = entry?.apy > 0.01 ? percent(entry.apy) : null;
...
async function fetchLoopRates({ wallets }) {
  ...
  if (!evmWallets.length && !solanaWallets.length) {
    return {
      ...
      coverage: { aave: AAVE_MARKETS, morpho: MORPHO_CHAINS, fluid: FLUID_CHAINS, kamino: [], jupiterLend: [] },
    };
  }
