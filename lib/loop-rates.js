...
function mapMorphoMarketPosition(wallet, chain, pos) {
  ...
  const collateralUsd = num(state.collateralUsd);
  const supplyUsd = num(state.supplyAssetsUsd);
  const borrowUsd = num(state.borrowAssetsUsd);
...
function mapMorphoVaultPosition(wallet, chain, pos, version) {
  ...
  const assetsUsd = num(pos?.state?.assetsUsd ?? pos?.assetsUsd);
...
async function fetchMorphoWalletChain(wallet, chain) {
  const query = `query MorphoLoops($address:String!, $chainId:Int!) {
    userByAddress(address:$address, chainId:$chainId) {
      ...
      vaultPositions {
        vault { address name asset { symbol address decimals } state { apy netApy avgNetApy } }
        state { assetsUsd }
      }
      vaultV2Positions {
        vault { address name asset { symbol address decimals } apy netApy avgNetApy }
        assetsUsd
        shares
      }
    }
