...
function mapMorphoMarketPosition(wallet, chain, pos) {
  ...
  return {
    ...
    lendingOnly: borrowUsd <= 0.01 && totalSupplied > 0.01,
...
function fluidVaultPositionId(wallet, chainId, vault, nft) {
  ...
  return `fluid-vault:${wallet}:${chainId}:${vaultRef || 'vault'}:${nftRef || '0'}`;
}
...
function shouldEnrichLegWithDefillama(chainId, leg, index) {
  const symbol = normalizeYieldSymbol(leg?.symbol);
  if (symbol && DEFILLAMA_NATIVE_PROJECTS[symbol] && canonicalNativeYieldApy(chainId, leg, index)) return true;
