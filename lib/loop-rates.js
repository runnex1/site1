    ...
    for (const chainBlock of chains || []) {
      for (const reward of chainBlock.rewards || []) {
        for (const breakdown of reward.breakdowns || []) {
          const unclaimedUsd = merklUnclaimedUsdFromBreakdown(reward, breakdown);
          if (unclaimedUsd <= 0.01) continue;

          const opportunity = oppById.get(`${walletKey}:${breakdown.opportunityId}`);
          const matches = opportunity
            ? (positions || []).filter(p => String(p.wallet || '').toLowerCase() === walletKey
              && positionMatchesMerklOpportunity(p, opportunity))
            : [];
          ...
          for (const position of matches) {
            ...
            earnedByPosition[position.id] = num(earnedByPosition[position.id]) + unclaimedUsd * share;
          }
        }
      }
    }
...
async function fetchLoopRates({ wallets }) {
  ...
  if (evmWallets.length) {
    ...
    positions.sort((a, b) => Math.abs(b.netValue || 0) - Math.abs(a.netValue || 0));
  }
