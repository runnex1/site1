...
function positionHasMerklExplorerRef(position) {
  return merklExplorerCandidates(position).some(isMerklExplorerRef);
}
...
function merklLookupMeta(index, side, wallet, chainId, address) {
  ...
  for (const key of keys) {
    const meta = bucket.byExplorer.get(key) || bucket.byUnderlying.get(key);
    if (!meta) continue;
...
function isPlainCollateralLeg(leg) {
  if (leg?.role === 'collateral') return true;
  const symbol = normalizeYieldSymbol(leg?.symbol);
  return Boolean(leg?.isCollateral && NON_YIELD_COLLATERAL_SYMBOLS.has(symbol));
}

function shouldEnrichLegWithDefillama(chainId, leg, index) {
  if (canonicalNativeYieldApy(chainId, leg, index)) return true;
  if (isPlainCollateralLeg(leg)) return false;
  return num(leg.apy, 0) <= 0.01;
}
...
function enrichPositionWithDefillamaYield(position, index) {
  ...
  for (const leg of position.supplied || []) {
    const dlApyRaw = defillamaApyForLeg(position.chainId, leg, index);
    if (!dlApyRaw || dlApyRaw <= 0.01) continue;
    if (!shouldEnrichLegWithDefillama(position.chainId, leg, index)) continue;
    const dlApy = percent(dlApyRaw);
...
function enrichPositionWithMerkl(position, merklIndex) {
  ...
  const hasMarketRef = positionHasMerklExplorerRef(position);
  for (const leg of position.borrowed || []) {
    ...
    if (!meta && !hasMarketRef) {
      meta = merklLookupMeta(merklIndex, 'borrow', position.wallet, position.chainId, leg.address);
    }
    if (!meta) continue;
