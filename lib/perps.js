...
function pushClosedPair(pairs, a, b) {
  ...
  pairs.push({
    ...
    sizeMismatchPct: closedLegSizeMismatchPct(a, b),
    longLeg,
    shortLeg,
    closeSlippage,
    funding,
...
function closedLegsCanPair(a, b) {
  if (closedLegSizeMismatchPct(a, b) > CLOSED_PAIR_MAX_SIZE_MISMATCH_PCT) return false;
    ...
    while (true) {
      ...
      for (let i = 0; i < symbolLegs.length; i++) {
        ...
        for (let j = 0; j < symbolLegs.length; j++) {
          ...
          const sizePenalty = closedLegSizeMismatchPct(a, b) * 60000;
