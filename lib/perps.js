...
function enrichClosedPairSessionPnl(pair, dailySeriesInputs, fillHistoryDays = PERPS_MAX_FILL_HISTORY_DAYS) {
  ...
  const filteredSeries = peakPair.peakMetricsApplied && windowStart > 0
    ? filterDailySeriesSince(sessionSeries, windowStart)
    : sessionSeries;
  ...
  if (peakPair.peakMetricsApplied) {
    const next = {
      ...peakPair,
      dailyPerformanceSeries: filteredSeries.length ? filteredSeries : sessionSeries,
      ...lifetimeFields,
    ...
    if (keepFrozenFunding) {
      ...
      next.netPnl = Number(next.closeSlippage || 0) + next.funding - Number(next.fees || 0);
    }
    return attachClosedPairSessionMetrics(next);
  }

  const totals = pairLatestSessionTotals(dailyPerformanceSeries);
