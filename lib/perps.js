...
function slimPerpsDashboardForClient(payload) {
  ...
  const slimFills = (block) => {
    ...
    const kept = fills.filter((f) => (Number(f?.time) || 0) >= cutoffMs);
    return {
      ...block,
      fills: kept,
  ...
  const slimMatches = (block) => {
    ...
    const kept = matches.filter((m) => (Number(m?.time) || 0) >= cutoffMs);
    return {
      ...block,
      matches: kept,
  ...
  return {
    ...
    hyperliquid: payload.hyperliquid
      ? { ...payload.hyperliquid, fills: slimFills(payload.hyperliquid.fills) }
      : payload.hyperliquid,
    nado: payload.nado
      ? { ...payload.nado, matches: slimMatches(payload.nado.matches) }
      : payload.nado,
    grvt: payload.grvt
      ? { ...payload.grvt, fills: slimFills(payload.grvt.fills) }
      : payload.grvt,
    extended: payload.extended
      ? { ...payload.extended, fills: slimFills(payload.extended.fills) }
      : payload.extended,
  };
}
