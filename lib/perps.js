  ...
  for (const symbolLegs of Object.values(bySymbol)) {
    ...
    symbolLegs.forEach((leg, idx) => {
      ...
      const coveredByHedgedPair = pairs.some((p) => {
        ...
        if (Math.abs(Number(p.closeTime || 0) - Number(leg.closeTime || 0)) > CLOSED_PAIR_SYNTHETIC_MATCH_WINDOW_MS) {
          return false;
        }
        return [p.longLeg, p.shortLeg].some((l) => l && l.venue === leg.venue);
      });
      if (coveredByHedgedPair) return;
      // Unhedged until close → Closed tab shows only that exchange leg.
