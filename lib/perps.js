...
/** Opposite legs of a hedge may close hours–days apart; still one hedged round if they overlapped. */
const CLOSED_PAIR_STAGGER_MATCH_WINDOW_MS = 7 * 86400000;
...
function closedPairLifetimeMetrics(pair) {
  ...
  if (pair?.exchangeOnly) {
    const solo = longLeg || shortLeg;
    if (!solo?.openTimeKnown) return null;
    const openTime = Number(solo.openTime || 0);
...
function closedLegsCanPair(a, b) {
  ...
  const canScale = closedLegCanScaleToMatch(a, b) || closedLegCanScaleToMatch(b, a);
  if (sizeMismatch > CLOSED_PAIR_MAX_SIZE_MISMATCH_PCT && !canScale) {
    return false;
  }
  const closeGap = Math.abs(a.closeTime - b.closeTime);
  if (closeGap <= CLOSED_PAIR_MATCH_WINDOW_MS) return true;
  if ((!a.openTimeKnown || !b.openTimeKnown) && closeGap <= CLOSED_PAIR_SYNTHETIC_MATCH_WINDOW_MS) {
    return true;
  }
  // Staggered hedge exit: one venue closed hours/days before the other, but the
  // positions overlapped (delta-neutral for most of the life).
  if (closeGap <= CLOSED_PAIR_STAGGER_MATCH_WINDOW_MS) {
    if (a.openTimeKnown && b.openTimeKnown) return closedLegOpenWindowsOverlap(a, b);
    // Unknown open times: same-day legacy path, or size-matched within stagger.
    if (closedLegsSameUtcCloseDay(a, b)) return true;
    return sizeMismatch <= CLOSED_PAIR_MAX_SIZE_MISMATCH_PCT || canScale;
  }
  return false;
}

/** True when an opposite venue/side closed nearby — was hedged, not never-hedged. */
function closedLegHasHedgeCounterpart(leg, symbolLegs) {
  return (symbolLegs || []).some((other) => {
    if (!other || other === leg) return false;
    if (other.venue === leg.venue || other.side === leg.side) return false;
    const closeGap = Math.abs(Number(other.closeTime || 0) - Number(leg.closeTime || 0));
    if (closeGap > CLOSED_PAIR_STAGGER_MATCH_WINDOW_MS) return false;
    // Known open windows: only count as hedged if they actually overlapped.
    if (leg.openTimeKnown && other.openTimeKnown) {
      return closedLegOpenWindowsOverlap(leg, other);
    }
    // Unknown opens + opposite venue within stagger → treat as staggered hedge leftover.
    return true;
  });
}
...
function buildClosedPairsFromLegs(legs) {
  ...
  for (const symbolLegs of Object.values(bySymbol)) {
    ...
    symbolLegs.forEach((leg, idx) => {
      ...
      unmatched.push(leg);
      // If a hedged pair already includes this venue near the same close, skip solo row
      // (avoids duplicate history leftovers after fill/history merge).
      const coveredByHedgedPair = pairs.some((p) => {
        if (p.exchangeOnly || p.symbol !== leg.symbol) return false;
        if (Math.abs(Number(p.closeTime || 0) - Number(leg.closeTime || 0)) > CLOSED_PAIR_STAGGER_MATCH_WINDOW_MS) {
          return false;
        }
        return [p.longLeg, p.shortLeg].some((l) => l && l.venue === leg.venue);
      });
      if (coveredByHedgedPair) return;
      // Solo Closed rows ONLY for never-hedged full rounds:
      // - not closing-fill fragments (partial exits / dust)
      // - not exchange position-history leftovers (usually Var / staggered hedge)
      // - no opposite-venue hedge counterpart in the stagger window
      if (!leg.openTimeKnown || leg.reconstructedFromClosingFills || leg.fromExchangeHistory) return;
      if (closedLegHasHedgeCounterpart(leg, symbolLegs)) return;
...
function buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase = {}) {
  const spreadRows = [];
...
async function fetchPerpsLiveRates(opts = {}) {
  ...
  const [hlRates, nadoRates, grvtRates, extendedRates, variationalRates] = await Promise.all([
  ...
  return {
    fetchedAt: Date.now(),
    rateSpread: buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase),
  };
}
...
async function fetchPerpsDashboard(wallets) {
  ...
  const [grvtRates, extendedRates, variationalRates] = await Promise.all([
    grvtEnabled
  ...
  const spreadRows = buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase);
...
function slimPerpsDashboardForClient(payload) {
  ...
  const fillCutoffMs = now - 45 * 86400000;
  const recentEventCutoffMs = now - 2 * 86400000;

  const recentDailyEventCutoffMs = now - 2 * 86400000;
  const slimDaily = (series) => (series || []).map((row) => {
    if (!row || typeof row !== 'object') return row;
    const ts = Number(row?.ts) || Date.parse(`${row?.day || ''}T12:00:00Z`) || 0;
    // Keep ~2d of per-event rows so 1D range trim can split the boundary day.
  ...
  const slimPairDaily = (series) => {
    ...
    // except the last ~2d (needed for accurate 1D boundary splits).
...
function appendEquitySnapshotStore(store, data, maxEntries = 180) {
  ...
  if (next[key]) {
    const prev = next[key];
    if (Number.isFinite(Number(prev.variationalEquityAdjust))) return next;
    next[key] = {
      ...prev,
...
async function fetchPerpsCapitalFlowsBundle(wallets) {
  ...
  const computed = computeCombinedNetDeposits(hl, nado, grvt, extended);
  return {
    hl,
    nado,
    grvt,
