...
/** When the hedge was opened: latest of the two legs' first fills (both sides live). */
function pairOpenedAtMs(base, venueA, venueB, fillSources) {
  if (!fillSources) return null;
...
function attachPairFundingMeta(pair, base, venueA, venueB, paymentSources, fillSources, fillHistoryDays) {
  ...
  pair.pairOpenedAtMs = pairOpenedAtMs(base, venueA, venueB, fillSources);
...
const CLOSED_PAIR_MATCH_WINDOW_MS = 36 * 3600000;
const CLOSED_SYNTHETIC_CLOSE_CLUSTER_MS = 4 * 3600000;
...
function buildClosedPairs(fillSources, paymentSources) {
  const legs = Object.entries(fillSources).flatMap(([venue, fills]) => buildClosedLegsForVenue(venue, fills, paymentSources));
  const used = new Set();
  const pairs = [];
  const sorted = legs.sort((a, b) => b.closeTime - a.closeTime);
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    for (let j = i + 1; j < sorted.length; j++) {
      ...
      const b = sorted[j];
      if (a.symbol !== b.symbol || a.venue === b.venue || a.side === b.side) continue;
...
async function fetchPerpsDashboard(wallets) {
  ...
  const [
    ...
    withTimeout(fetchNadoFunding(nadoWallet, fillHistoryDays, 'default', activeNadoSymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO funding'),
    withTimeout(fetchHyperliquidFills(hlWallet, fillHistoryDays), PERPS_CORE_FETCH_TIMEOUT_MS, 'Hyperliquid fills'),
    withTimeout(fetchNadoMatches(nadoWallet, fillHistoryDays, 'default', activeNadoSymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO matches'),
    withTimeout(fetchHyperliquidRates(), PERPS_CORE_FETCH_TIMEOUT_MS, 'Hyperliquid rates'),
    withTimeout(fetchNadoRates(), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO rates'),
  ...
  const nadoHistorySymbols = Array.from(new Set([
  ...
  const needsNadoClosedHistory = nadoHistorySymbols.some(symbol => !activeNadoBaseSet.has(symbol));
