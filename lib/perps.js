...
function nadoHealthEquity(health) {
  if (!health) return null;
  const equity = fromX18(health.health);
  if (Number.isFinite(equity)) return equity;
  const assets = fromX18(health.assets || health.asset_value);
  const liabilities = fromX18(health.liabilities || 0);
...
async function fetchNadoState(wallet, subaccountName = 'default') {
  ...
  return {
    ...
    subaccount,
    exists: !!info.exists,
    accountValue: nadoHealthEquity(health),
