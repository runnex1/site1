...
function buildPairedAnalysis({
   ...
   * activity session, per venue. HL uses the price-based incremental realized
   * (realizes only on size decreases — never on position increases). Extended
   * uses exchange-reported realisedPnl: the live open position carries the
   * accumulated partial-close realized, history rows add fully-closed sessions.
   */
  function attachSessionRealizedByVenue(pair, base) {
    ...
    let ext = Number(extendedByBase[base]?.realisedPnl) || 0;
    for (const pos of extendedPositionHistory?.positions || []) {
      if (toBaseSymbol(extendedBaseFromMarket(pos.market)) !== base) continue;
      const ct = Number(extendedClosedTimeMs(pos)) || 0;
      if (ct >= sinceMs) ext += Number(pos.realisedPnl) || 0;
    }
    out.extended = ext;
