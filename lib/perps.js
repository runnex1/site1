...
function closedLegsCanPair(a, b) {
  ...
  const timeCompatible =
    closeGap <= CLOSED_PAIR_MATCH_WINDOW_MS
    || ((!a.openTimeKnown || !b.openTimeKnown) && closeGap <= CLOSED_PAIR_SYNTHETIC_MATCH_WINDOW_MS)
    || (closedLegsSameUtcCloseDay(a, b)
        && (!a.openTimeKnown || !b.openTimeKnown || closedLegOpenWindowsOverlap(a, b)));
  if (!timeCompatible) return false;
  // Time-compatible opposite-venue legs are the same position even when sizes differ
  // (partially hedged / uneven hedge). Pair them into one row instead of leaving them
  // unmatched — a partially hedged position must never show as a SOLO row. The greedy
  // scorer still prefers well-matched sizes, so exact hedges pair first.
  return true;
}
...
function closedLegIsNeverHedgedSolo(leg, symbolLegs, nowMs = Date.now()) {
  ...
  // A partially hedged position (opposite-venue activity overlapping the leg) must
  // pair instead of becoming a SOLO row — never show a hedge as solo.
...
function buildClosedPairsFromLegs(legs) {
  ...
  for (const symbolLegs of Object.values(bySymbol)) {
    ...
    symbolLegs.forEach((leg, idx) => {
      ...
      unmatched.push(leg);
      // Only genuinely unhedged positions (no opposite-venue activity) become SOLO.
      // Partially hedged positions must pair instead — never show as solo.
