...
async function reconstructGrvtSymbolSession({
  ...
  const fromHistory = latestClosed ? {
    ...
    // Raw GRVT field — opposite sign from our funding_payment_history trader cashflow.
    fundingRaw: latestClosed.funding,
  ...
  if (fromHistory) {
    verdict = {
      ...
      funding: (Math.abs(Number(fromHistory.funding)) > 1e-9 || !fromFills)
        ? fromHistory.funding
        : fromFills.funding,
      note: 'GRVT position_history closed row is the authoritative latest-session exchange leg',
    };
  } else if (fromFills) {
    verdict = {
      source: 'fill_history',
...
async function fetchPerpsEquitySnapshotWithVariational(wallets, opts = {}) {
  ...
  const [equityBase, hlState, nadoState, grvtState, extendedState] = await Promise.all([
