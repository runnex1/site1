...
async function extendedPaginate(path, params, windowStartMs) {
  ...
  for (let page = 0; page < 50; page++) {
    ...
    if (!cursor || batch.length < (params.limit || 500)) break;
  }
...
async function fetchExtendedFunding(days = 30) {
  ...
  const payments = rows.map(row => ({
    ...
    usdc: -parseFloat(row.fundingFee || 0),
...
function buildFundingCumulativeSeries(hlPayments, nadoPayments, days, pairedBases = null, grvtPayments = null, extendedPayments = null) {
  ...
  if (extendedPayments) add(extendedPayments);
...
function buildPairedAnalysis({
  ...
  const fundingExtended = extendedFunding ? sumByBase(extendedFunding.payments, 'usdc') : {};
  ...
  function pushCrossPair(base, pairType, pairLabel, legA, legB, venueA, venueB, fundingA, fundingB, feesA, feesB, spreadKey) {
    ...
    const fundingWindow = (fundingHl[base] || 0) + (fundingNado[base] || 0)
      + (fundingGrvt[base] || 0) + (fundingExtended[base] || 0);
    const fees = (feesA || 0) + (feesB || 0);
    const spread = spreadByBase[base];
    const currentSpread8h = spread?.[spreadKey] ?? spread?.spread8h ?? null;
    const alerts = [];
    if (sizeMismatchPct > 1) alerts.push('size_mismatch');
    if (Math.abs(combinedUpnl) > 500) alerts.push('basis_drift');
    paired.push({
  ...
  for (const base of [...allBases].sort()) {
    ...
    if (hl && na) pushHlNadoPair(base, hl, na);
  }

  for (const base of [...allBases].sort()) {
    const na = nadoByBase[base];
    const ex = extendedByBase[base];
    if (na && ex && !isHedged(base, 'nado') && !isHedged(base, 'extended')) {
      pushCrossPair(
        base, 'nado_extended', 'Nado + Extended', na, ex, 'nado', 'extended',
        na.fundingSinceOpen, fundingExtended[base] ?? 0,
        feesNado[base], feesExtended[base], 'spread8h',
      );
    }
  }
  ...
  for (const base of [...allBases].sort()) {
    ...
    if (hl && gv && !isHedged(base, 'hyperliquid') && !isHedged(base, 'grvt')) {
      pushCrossPair(
        base, 'hl_grvt', 'HL + GRVT', hl, gv, 'hyperliquid', 'grvt',
        hlFundingSinceOpen(hl), grvtFundingSinceOpen(gv) ?? fundingGrvt[base] ?? 0,
        feesHl[base], feesGrvt[base], 'spreadHlGrvt8h',
      );
    }
  }
  ...
  for (const base of [...allBases].sort()) {
    const hl = hlByBase[base];
    const ex = extendedByBase[base];
    if (hl && ex && !isHedged(base, 'hyperliquid') && !isHedged(base, 'extended')) {
      pushCrossPair(
        base, 'hl_extended', 'HL + Extended', hl, ex, 'hyperliquid', 'extended',
        hlFundingSinceOpen(hl), fundingExtended[base] ?? 0,
        feesHl[base], feesExtended[base], 'spreadHlExtended8h',
      );
    }
  }
  ...
  const legSpecs = [
    ...
    { venue: 'extended', map: extendedByBase, funding: (p, b) => fundingExtended[b] ?? 0, fees: feesExtended },
  ];
...
async function fetchPerpsDashboard(wallets) {
  ...
  return {
    ...
    summary: {
      ...
      extendedFundingTotal: extendedFunding.totalFunding,
