...
async function fetchNadoMatches(wallet, days = 30, subaccountName = 'default', symbols = null) {
  ...
  // Fetch matches per product. NADO's archive may return only the latest page across
  // a product chunk, so busy markets can crowd out quieter closed rounds like MEGA.
  for (const productId of productIds) {
    await fetchMatchChunk([productId]);
  }
...
function buildPairedAnalysis({
  ...
  const closedPairs = closedResult.pairs;
...
async function fetchPerpsDashboard(wallets) {
  ...
  const [
    ...
    withTimeout(fetchHyperliquidRates(), PERPS_CORE_FETCH_TIMEOUT_MS, 'Hyperliquid rates'),
    withTimeout(fetchNadoRates(), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO rates').catch(() => []),
