...
function netDepositsAtTime(payments, timeMs) {
  return payments.reduce((s, p) => (p.time <= timeMs ? s + p.usdc : s), 0);
}
...
function buildEquitySeries({
  ...
  const allPayments = [
    ...(capitalFlows?.hl?.payments || []),
    ...(capitalFlows?.nado?.payments || []),
  ].sort((a, b) => a.time - b.time);
  const combinedNetDeposits = capitalFlows?.combinedNetDeposits
    ?? allPayments.reduce((s, p) => s + p.usdc, 0);
  ...
  for (const bf of backfill) {
    ...
    const cumulativeNetDeposits = bf.cumulativeNetDeposits ?? netDepositsAtTime(allPayments, time);
    points.push({
      time,
  ...
  for (const snap of snapshots) {
    ...
    const cumulativeNetDeposits = snap.cumulativeNetDeposits ?? netDepositsAtTime(allPayments, time);
    points.push({
      time,
...
async function fetchPerpsDashboard(wallets) {
  ...
  const capitalFlows = {
    ...
    combinedNetDeposits: hlCapitalFlows.netDeposits + nadoCapitalFlows.netDeposits,
  };
  const equityNow = {
