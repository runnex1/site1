...
async function fetchGrvtPositionHistory(subAccountId, days = 30) {
  const empty = { venue: 'grvt', subAccountId, days, positions: [] };
  if (!subAccountId || !process.env.GRVT_API_KEY) return empty;

  const windowStartNs = String((Date.now() - days * 86400000) * 1e6);
  const positions = [];
  let cursor = '';
  for (let page = 0; page < 50; page++) {
    const data = await grvtTradesPost('position_history', {
      sub_account_id: String(subAccountId),
      kind: ['PERPETUAL'],
      start_time: windowStartNs,
      limit: 500,
      cursor,
      status: ['CLOSED', 'LIQUIDATED', 'SETTLED'],
    });
    const batch = data.result || [];
  ...
  return { venue: 'grvt', subAccountId, days, positions };
}
...
const CLOSED_PAIR_MATCH_WINDOW_MS = 7 * 86400000;
...
function buildClosedPairs(fillSources, paymentSources, exchangeHistory = null) {
  const useGrvtHistory = Boolean(exchangeHistory?.grvt?.length);
  ...
  const exchangeLegs = exchangeHistory
    ...
    : [];
  const fillLegs = Object.entries(fillSources).flatMap(([venue, fills]) => {
    if (venue === 'grvt' && useGrvtHistory) return [];
    if (venue === 'extended' && useExtendedHistory) return [];
    return buildClosedLegsForVenue(venue, fills, paymentSources);
  });
  const legs = [...exchangeLegs, ...fillLegs];
  const bySymbol = {};
...
async function fetchPerpsDashboard(wallets) {
  ...
  const [
    ...
    grvtEnabled ? withTimeout(fetchGrvtFills(grvtSubAccount, fillHistoryDays), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'GRVT fills').catch(e => ({
      venue: 'grvt', subAccountId: grvtSubAccount, days, fills: [], totalFees: 0, totalRealized: 0, error: errorMessage(e),
    })) : Promise.resolve({ venue: 'grvt', subAccountId: grvtSubAccount, days, fills: [], totalFees: 0, totalRealized: 0 }),
    grvtEnabled ? withTimeout(fetchGrvtPositionHistory(grvtSubAccount, fillHistoryDays), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'GRVT position history').catch(e => ({
      venue: 'grvt', subAccountId: grvtSubAccount, days, positions: [], error: errorMessage(e),
    })) : Promise.resolve({ venue: 'grvt', subAccountId: grvtSubAccount, days, positions: [] }),
  ...
  if (needsNadoClosedHistory) {
    const [nf, nm] = await Promise.all([
      withTimeout(fetchNadoFunding(nadoWallet, fillHistoryDays, 'default', nadoHistorySymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO funding history').catch(e => ({
        ...nadoFunding,
        error: errorMessage(e),
      })),
      withTimeout(fetchNadoMatches(nadoWallet, fillHistoryDays, 'default', nadoHistorySymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO match history').catch(e => ({
        ...nadoMatches,
        error: errorMessage(e),
      })),
    ]);
  ...
  return {
    ...
    summary: {
      ...
      grvtError: combineErrors(grvtState, grvtFunding, grvtFills, grvtCapitalFlows),
