...
function normalizeGrvtPositionOverride(row) {
  ...
  if (row.venue === 'grvt' && row.symbol && (row.side || Number.isFinite(Number(row.size)))) {
    return {
      ...
      side: row.side || (Number(row.size) >= 0 ? 'long' : 'short'),
...
/** Close slippage per UTC day from fully closed hedged pairs (booked on close day). */
function buildCloseSlippageByDay(closedPairs) {
  const byDay = {};
  for (const pair of closedPairs || []) {
    const closeTime = Number(pair.closeTime || 0);
    if (!closeTime) continue;
    const day = isoDateFromMs(closeTime);
    byDay[day] = (byDay[day] || 0) + Number(pair.closeSlippage || 0);
  }
  return byDay;
}

function mergeCloseSlippageIntoDailySeries(series, slippageByDay) {
  const slip = slippageByDay || {};
  let cumFunding = 0;
  let cumFees = 0;
  let cumSlippage = 0;
  let cumNet = 0;
  return (series || []).map(row => {
    ...
    const dailySlippage = slip[row.day] || 0;
    const dailyNet = dailyFunding - dailyFees + dailySlippage;
    cumFunding += dailyFunding;
    cumFees += dailyFees;
    cumSlippage += dailySlippage;
    cumNet += dailyNet;
    return {
      ...row,
      dailySlippage,
...
function sumVenueRealizedClosePnlSince(base, venue, fillSources, sinceMs) {
  let sum = 0;
  for (const item of fillSources?.[venue] || []) {
    if (toBaseSymbol(item.symbol) !== base) continue;
    const time = Number(item.time || 0);
    if (sinceMs && time < sinceMs) continue;
    const pnl = Number(item.closedPnl ?? item.realizedPnl ?? 0);
    if (!Number.isFinite(pnl) || Math.abs(pnl) <= CLOSED_ROUND_EPS) continue;
    sum += pnl;
  }
  return sum;
}

function sumPairSessionRealizedSlippage(base, venueA, venueB, fillSources, sinceMs) {
  return sumVenueRealizedClosePnlSince(base, venueA, fillSources, sinceMs)
    + sumVenueRealizedClosePnlSince(base, venueB, fillSources, sinceMs);
}

/** Only count closing PnL on days both hedged legs realized a close (partial or full). */
function sumPairHedgedCloseSlippageSince(base, venueA, venueB, fillSources, sinceMs) {
  const byDay = {};
  for (const venue of [venueA, venueB]) {
    for (const item of fillSources?.[venue] || []) {
      ...
      byDay[day][venue] = (byDay[day][venue] || 0) + pnl;
    }
  }
  let sum = 0;
  for (const row of Object.values(byDay)) {
    const pnlA = row[venueA];
    const pnlB = row[venueB];
    if (pnlA == null || pnlB == null) continue;
    if (Math.abs(pnlA) <= CLOSED_ROUND_EPS && Math.abs(pnlB) <= CLOSED_ROUND_EPS) continue;
    sum += pnlA + pnlB;
  }
  return sum;
}

/** Match Position Performance / Total PnL: latest session starts on first active funding day. */
function pairLatestSessionStartMsFromDailySeries(dailyPerformanceSeries) {
  const sessionRows = trimDailySeriesToLatestSession(dailyPerformanceSeries || []);
  const firstDay = sessionRows[0]?.day;
  if (!firstDay) return null;
  const ms = Date.parse(`${firstDay}T00:00:00.000Z`);
  return Number.isFinite(ms) ? ms : null;
}

function applyPairSessionRealizedSlippage(pair, fillSources) {
  const venueA = pair?.venueA;
  const venueB = pair?.venueB;
  const base = toBaseSymbol(pair?.symbol);
  if (!base || !venueA || !venueB) {
    pair.sessionRealizedSlippage = 0;
    pair.sessionStartMs = null;
    return pair;
  }
  const sessionStartMs = pairLatestSessionStartMsFromDailySeries(pair.dailyPerformanceSeries);
  pair.sessionStartMs = sessionStartMs;
  pair.sessionRealizedSlippage = sessionStartMs != null
    ? sumPairHedgedCloseSlippageSince(base, venueA, venueB, fillSources, sessionStartMs)
    : 0;
  return pair;
}
...
function buildPairedAnalysis({
  ...
  function pushCrossPair(base, pairType, pairLabel, legA, legB, venueA, venueB, fundingA, fundingB, feesA, feesB, spreadKey) {
    ...
    const realizedMaps = {
      hyperliquid: realizedHl,
      nado: realizedNado,
      grvt: realizedGrvt,
      extended: realizedExtended,
    };
    const realized = (realizedMaps[venueA]?.[base] || 0) + (realizedMaps[venueB]?.[base] || 0);
    const spread = spreadByBase[base];
    const currentSpread8h = netFundingSpread8h(
      legA.size,
...
async function fetchPerpsDashboard(wallets) {
  ...
  const closeSlippageByDay = buildCloseSlippageByDay(arb.closedPairs);
  const dailyFundingSeries = mergeCloseSlippageIntoDailySeries(
    buildDailyFundingSeries(dailySeriesInputs),
    closeSlippageByDay,
  );
  const fillSources = {
    hyperliquid: hlFills.fills || [],
    nado: nadoMatchesForAnalysis.matches || [],
    grvt: grvtFills.fills || [],
    extended: extendedFills.fills || [],
  };
  for (const p of arb.paired) {
    ...
    applyPairSessionRealizedSlippage(p, fillSources);
  }
  ...
  const pairedDailyFundingSeries = mergeCloseSlippageIntoDailySeries(
