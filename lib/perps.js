...
async function grvtAuth() {
  ...
  if (!r.ok) throw new Error(`GRVT auth HTTP ${r.status}`);
  const setCookie = r.headers.get('set-cookie') || '';
  const cookieMatch = setCookie.match(/gravity=([^;]+)/i);
  const accountId = r.headers.get('x-grvt-account-id') || '';
  if (!cookieMatch || !accountId) {
    const body = await r.json().catch(() => ({}));
    throw new Error(body.message || body.error || 'GRVT auth failed');
  }
  _grvtAuthCache = {
    cookie: `gravity=${cookieMatch[1]}`,
    accountId,
...
async function grvtTradesPost(path, body) {
  ...
  if (!r.ok) throw new Error(`GRVT ${path} HTTP ${r.status}`);
  const data = await r.json();
  if (data.error || data.code) throw new Error(data.message || data.error || `GRVT ${path} failed`);
  return data;
}
  ...
  const positions = (acc.positions || [])
    ...
    .map(p => {
      ...
      return {
        ...
        entryPx: parseFloat(p.entry_price || 0),
        markPx: parseFloat(p.mark_price || 0),
...
async function fetchGrvtRates(bases = []) {
  ...
  const rows = await Promise.all([...symbols].map(async base => {
    ...
    try {
      ...
      const raw8h = t.funding_rate_8h_curr ?? t.funding_rate_8h_avg ?? t.funding_rate;
      return {
        venue: 'grvt',
        symbol: base,
        instrument,
        fundingRate8h: grvtRateToDecimal(raw8h),
...
function computeCombinedNetDeposits(hlCapitalFlows, nadoCapitalFlows, grvtCapitalFlows = null, windowMs = CROSS_VENUE_WINDOW_MS) {
  ...
  const rawCombinedNetDeposits = hlNetDeposits + nadoNetDeposits + grvtNetDeposits;

  let crossVenueOffset = computeCrossVenueOffset(nadoP, hlP, windowMs);
  if (grvtCapitalFlows) {
    crossVenueOffset += computeCrossVenueOffset(grvtP, hlP, windowMs);
    crossVenueOffset += computeCrossVenueOffset(grvtP, nadoP, windowMs);
  }
...
function netDepositsAtTime(hlPayments, nadoPayments, timeMs, grvtPayments = null, windowMs = CROSS_VENUE_WINDOW_MS) {
...
function buildEquitySeries({
  ...
  for (const bf of backfill) {
    ...
    const totalEquity = bf.totalEquity ?? ((hl ?? 0) + (nado ?? 0) + grvt);
    const cumulativeNetDeposits = bf.cumulativeNetDeposits
      ?? netDepositsAtTime(hlPayments, nadoPayments, time, grvtPayments.length ? grvtPayments : null);
    points.push({
      time,
  ...
  for (const snap of snapshots) {
    ...
    const totalEquity = snap.totalEquity ?? hl + nado + grvt;
  ...
  const totalNow = hlNow + nadoNow + grvtNow;
  points.push({
    time: fetchedAt,
...
function buildFundingCumulativeSeries(hlPayments, nadoPayments, days, pairedBases = null, grvtPayments = null) {
  const dayMap = {};
  ...
  add(hlPayments);
  add(nadoPayments);
  if (grvtPayments) add(grvtPayments);
...
function buildPairedAnalysis({
  ...
  for (const base of [...allBases].sort()) {
    ...
    if (hl && na) {
      ...
      unhedged.push({
        ...
        size: gv.size,
        side: gv.side,
        notional: gv.notional,
        unrealizedPnl: gv.unrealizedPnl,
        funding: grvtFundingSinceOpen(gv) ?? fundingGrvt[base] ?? 0,
        fees: feesGrvt[base] || 0,
      });
    }
  }
  ...
  const totalFees = hlFills.totalFees + nadoMatches.totalFees + (grvtFills?.totalFees || 0);
  const totalRealized = hlFills.totalRealized + nadoMatches.totalRealized + (grvtFills?.totalRealized || 0);
  const totalEntrySlippage = paired.reduce((s, p) => s + (p.entrySlippage || 0), 0);
  const netFunding = hlFunding.totalFunding + nadoFunding.totalFunding + (grvtFunding?.totalFunding || 0);
...
async function fetchPerpsDashboard(wallets) {
  ...
  const [
    ...
    grvtEnabled ? fetchGrvtFills(grvtSubAccount, days).catch(() => ({
  ...
  const grvtRates = grvtEnabled
    ? await fetchGrvtRates(grvtState.positions.map(p => p.symbol)).catch(() => [])
    : [];
  ...
  const capitalFlows = {
    ...
    ...computeCombinedNetDeposits(hlCapitalFlows, nadoCapitalFlows, grvtCapitalFlows),
  };
  ...
  const equityNow = {
    ...
    grvt: grvtEquity,
    total: hlState.accountValue + (nadoState.accountValue ?? 0) + grvtEquity,
    adjustedTotal: hlState.accountValue + (nadoState.accountValue ?? 0) + grvtEquity - capitalFlows.combinedNetDeposits,
  };
  ...
  return {
    ...
    summary: {
      ...
      netFundingTotal: hlFunding.totalFunding + nadoFunding.totalFunding + grvtFunding.totalFunding,
      ...
      netFundingTotalAllAccounts: hlFunding.totalFunding + nadoFunding.totalFunding + grvtFunding.totalFunding,
