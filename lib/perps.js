...
function nadoLiquidationPriceFrom(balanceRow, ctx = null) {
  ...
  if (direct != null) return direct;
  if (!ctx) return null;
  const primary = computeNadoLiquidationPx(ctx);
  if (primary != null) return primary;
  if (ctx.proportionalHealth > 0) {
    return computeNadoLiquidationPx({
      ...ctx,
      maintenanceHealth: ctx.proportionalHealth,
    });
  }
  return null;
}

function estimateHyperliquidLiquidationPx(pos, markPx = null) {
  const direct = liquidationPriceFrom(pos);
  if (direct != null) return direct;
  const szi = parseFloat(pos?.szi ?? pos?.size ?? 0);
  if (!szi) return null;
  const side = szi >= 0 ? 1 : -1;
  const mark = Number(markPx ?? pos?.markPx ?? pos?.entryPx ?? 0);
  const marginUsed = parseFloat(pos?.marginUsed ?? 0);
  const maxLev = parseFloat(pos?.maxLeverage ?? pos?.leverage?.value ?? 0);
  if (!mark || !marginUsed || !maxLev) return null;
  const maintenanceLeverage = maxLev * 2;
  const l = 1 / maintenanceLeverage;
  const est = mark - side * marginUsed / Math.abs(szi) / (1 - l * side);
  return est > 0 && Number.isFinite(est) ? est : null;
}
...
function estimateGrvtLiquidationPx(row, accountValue = null) {
  const p = normalizeGrvtPositionRow(row);
  const direct = liquidationPriceFrom(p, grvtPx);
  if (direct != null) return direct;
  const size = parseFloat(p.size || 0);
  if (!size) return null;
  const side = size >= 0 ? 1 : -1;
  const mark = grvtPx(p.mark_price);
  const leverage = parseFloat(p.leverage || 0);
  const notional = grvtNotionalUsd(p, size, mark);
  if (!mark || !leverage) return null;
  const isolatedMm = parseFloat(p.isolated_mm || 0);
  const marginUsed = isolatedMm > 0
    ? isolatedMm
    : (notional / leverage);
  const maintenanceLeverage = leverage * 2;
  const l = 1 / maintenanceLeverage;
  let est = mark - side * marginUsed / Math.abs(size) / (1 - l * side);
  if (!(est > 0 && Number.isFinite(est)) && accountValue > 0 && notional > 0) {
    const crossMargin = Math.max(accountValue * (notional / (accountValue + notional)), marginUsed);
    est = mark - side * crossMargin / Math.abs(size) / (1 - l * side);
  }
  return est > 0 && Number.isFinite(est) ? est : null;
}

function mapGrvtPositions(rows, accountValue = null) {
  return (rows || [])
    ...
    .map(p => {
      ...
      return {
        ...
        markPx,
        liquidationPx: estimateGrvtLiquidationPx(p, accountValue),
...
async function fetchGrvtState(subAccountId) {
  ...
  const positions = mapGrvtPositions(positionRows, accountValue);
...
async function fetchHyperliquidState(wallet) {
  const [state, spotState, assetCtxs] = await Promise.all([
    hlPost({ type: 'clearinghouseState', user: wallet }),
    hlPost({ type: 'spotClearinghouseState', user: wallet }).catch(() => ({ balances: [] })),
  ...
  const positions = (state.assetPositions || [])
    ...
    .map(p => {
      ...
      return {
        ...
        markPx,
        liquidationPx: estimateHyperliquidLiquidationPx({
          ...pos,
          maxLeverage: pos.maxLeverage ?? pos.leverage?.value,
        }, markPx),
...
async function fetchNadoState(wallet, subaccountName = 'default') {
  ...
  const perpNotionalTotal = openBalances.reduce((sum, b) => {
    const amount = fromX18(b.balance?.amount);
    const oracle = oracleByProduct[b.product_id];
    return sum + (oracle != null ? Math.abs(amount * oracle) : 0);
  }, 0);
  const positions = openBalances
    .map(b => {
      ...
      const positionNotional = oracle != null ? Math.abs(amount * oracle) : 0;
      const proportionalHealth = perpNotionalTotal > 0 && maintenanceHealth > 0
        ? maintenanceHealth * (positionNotional / perpNotionalTotal)
        : null;
  ...
  return {
    ...
    positions,
    error: pnlByProduct.__error ? `NADO position events unavailable: ${pnlByProduct.__error}` : null,
  };
}
...
function buildPairedAnalysis({
  ...
  const closedPairs = filterFullyClosedPairs(closedResult.pairs, {
