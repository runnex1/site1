 ...
 * If GRVT geo-blocks the host IP, set GRVT_PROXY_URL (or HTTPS_PROXY) in Vercel env.
 */
...
let _grvtProxyAgent = null;
function grvtProxyUrl() {
  return String(process.env.GRVT_PROXY_URL || process.env.HTTPS_PROXY || '').trim();
}
function grvtProxyAgent() {
  const proxyUrl = grvtProxyUrl();
  if (!proxyUrl) return null;
  if (!_grvtProxyAgent || _grvtProxyAgent.proxyUrl !== proxyUrl) {
    const { ProxyAgent } = require('undici');
    _grvtProxyAgent = { proxyUrl, agent: new ProxyAgent(proxyUrl) };
  }
  return _grvtProxyAgent.agent;
}
async function grvtFetch(url, opts = {}, ms = PERPS_OPTIONAL_FETCH_TIMEOUT_MS, label = 'GRVT fetch') {
  const agent = grvtProxyAgent();
