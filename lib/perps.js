...
async function extendedGet(path, params = {}) {
  ...
  const r = await fetchWithTimeout(url, {
    // Extended requires a User-Agent header on every REST request (see API docs
    // "Mandatory headers"); read-only endpoints also require X-Api-Key.
    headers: {
      'X-Api-Key': apiKey,
      Accept: 'application/json',
      'User-Agent': 'vault-portfolio/2.0 (+https://testedefi.vercel.app)',
    },
...
function sanitizeEquitySnapshotStore(store, defaultBaselineDeposit = null) {
  if (!store || typeof store !== 'object') return {};
  const entries = Object.entries(store)
    .filter(([k, v]) => v && typeof v === 'object' && !k.startsWith('_'))
    .sort((a, b) => (Number(a[1]?.fetchedAt) || parseEquityBucketTime(a[0])) - (Number(b[1]?.fetchedAt) || parseEquityBucketTime(b[0])));
  if (!entries.length) return {};

  // Find the prevailing valid deposit baseline from valid snapshots
  let validDepositBaseline = defaultBaselineDeposit;
  for (const [, snap] of entries) {
    const dep = Number(snap.cumulativeNetDeposits);
    if (Number.isFinite(dep) && dep > 0) {
      validDepositBaseline = dep;
      break;
    }
  }
  ...
  for (const [key, snap] of entries) {
    const totalEquity = Number(snap.totalEquity);
    if (!Number.isFinite(totalEquity) || totalEquity <= 0) continue;

    let dep = Number(snap.cumulativeNetDeposits);
    // If snapshot has 0 or missing deposits but baseline deposits exist, repair it
    if ((!Number.isFinite(dep) || dep <= 0) && Number.isFinite(validDepositBaseline) && validDepositBaseline > 0) {
      dep = validDepositBaseline;
    } else if (Number.isFinite(dep) && dep > 0) {
      validDepositBaseline = dep;
    }

    const adj = Number.isFinite(dep) ? totalEquity - dep : Number(snap.adjustedEquity) || totalEquity;
...
function buildEquitySeries({
  ...
  const flowsHealthy = Boolean(
    !capitalFlows?.hasError
    && !capitalFlows?.hl?.error
    && !capitalFlows?.nado?.error
    && (hlPayments.length || nadoPayments.length),
  );
  ...
  for (const bf of backfill) {
    ...
    let cumulativeNetDeposits = bf.cumulativeNetDeposits;
    if (flowsHealthy) {
      const calculated = netDepositsAtTime(
        hlPayments,
        nadoPayments,
        time,
      ...
      if (Number.isFinite(calculated) && calculated > 0) {
        cumulativeNetDeposits = calculated;
      }
    }
    if (!Number.isFinite(cumulativeNetDeposits) || cumulativeNetDeposits <= 0) {
      cumulativeNetDeposits = combinedNetDeposits || 0;
    }
    points.push({
      time,
  ...
  for (const snap of snapshots) {
    ...
    if (totalEquity <= 0) continue;

    let cumulativeNetDeposits = Number(snap.cumulativeNetDeposits);
    if (flowsHealthy) {
      const calculated = netDepositsAtTime(
        hlPayments,
        nadoPayments,
    ...
    const adjustedEquity = totalEquity - cumulativeNetDeposits;
    points.push({
      time,
...
function closedLegIsNeverHedgedSolo(leg, allLegs = [], varHedges = []) {
  if (!leg || !leg.symbol || !leg.closeTime) return false;
  const sym = toBaseSymbol(leg.symbol);
  const open = Number(leg.openTime || 0);
  const close = Number(leg.closeTime || 0);
  if (varHedges.some((h) => toBaseSymbol(h.symbol) === sym && h.trackedVenue === leg.venue)) return false;
  return true;
}
...
function buildPairedAnalysis({
   ...
   *  Allow up to 90% mismatch so a large residual leg (e.g. HL short 475k vs Nado long 110k =
   *  76.8%) still pairs with a size_mismatch alert + a "hedge partial" action instead of
   *  appearing fully unhedged. Only true dust (e.g. 10 vs 0.2 = 98%) stays unpaired. */
  const OPEN_PAIR_MAX_SIZE_MISMATCH_PCT = 90;
...
function isEquitySnapshotEligible(data) {
  ...
  const hlVal = s.hlAccountValue ?? data?.equityNow?.hl;
  const nadoVal = s.nadoAccountValue ?? data?.equityNow?.nado;
  if (!Number.isFinite(hlVal) || !Number.isFinite(nadoVal)) return false;

  const total = s.adjustedEquity ?? data?.equityNow?.total ?? (hlVal + nadoVal);
  if (!Number.isFinite(total) || total <= 0) return false;

  const values = [hlVal, nadoVal];
...
function repairEquitySnapshotDeposits(store, capitalFlows) {
  ...
  // If capital flows bundle encountered API timeouts or failures, NEVER rewrite historical snapshots
  if (capitalFlows.hasError || capitalFlows.hl?.error || capitalFlows.nado?.error) {
    return { store: store || {}, changed: 0 };
  }
  const hlPayments = capitalFlows.hl?.payments || [];
  const nadoPayments = capitalFlows.nado?.payments || [];
  if (!hlPayments.length && !nadoPayments.length) {
    return { store: store || {}, changed: 0 };
  }
  ...
  for (const [key, snap] of Object.entries(next)) {
    ...
    // Never allow a live recalculation to collapse an existing positive baseline deposit to zero
    if (dep <= 0 && prevDep > 0) continue;
...
async function fetchPerpsCapitalFlowsBundle(wallets) {
  ...
  const hasError = Boolean(
    ...
    || (phoenix && phoenix.error)
    || (perplFlows && perplFlows.error),
  );

  const computed = computeCombinedNetDeposits(hl, nado, grvt, extended, phoenix, perplFlows);
  let finalCombinedNetDeposits = computed.combinedNetDeposits;
  // If an API timed out but a previous verified cumulative deposit baseline exists, retain it
  if (hasError && Number.isFinite(wallets.cumulativeNetDeposits) && wallets.cumulativeNetDeposits > 0 && finalCombinedNetDeposits <= 0) {
    finalCombinedNetDeposits = wallets.cumulativeNetDeposits;
  }
  ...
  return {
    ...
    hasError,
    ...computed,
    combinedNetDeposits: finalCombinedNetDeposits,
  };
}
