...
/** Closed-pair stats from 24h position peak through close (fills + funding). */
function enrichClosedPairSessionPnl(pair, dailySeriesInputs, fillHistoryDays = PERPS_MAX_FILL_HISTORY_DAYS) {
  ...
  const windowStart = Number(peakPair.statsSinceMs || peakPair.openTime || 0);
  const openDays = windowStart > 0
    ? Math.ceil((closeTime - windowStart) / 86400000) + 2
    : fillHistoryDays;
  ...
  const filteredSeries = peakPair.peakMetricsApplied
    ? filterDailySeriesSince(dailyPerformanceSeries, windowStart)
  ...
  // Variational closes freeze total funding on the hedge — do not replace with exchange-only session sums.
  const keepFrozenFunding = Boolean(pair?.manualVariationalClose)
    && Number.isFinite(Number(peakPair.funding ?? pair.funding));

  const totals = pairLatestSessionTotals(filteredSeries);
  if (peakPair.peakMetricsApplied) {
    const next = {
      ...peakPair,
      dailyPerformanceSeries: filteredSeries,
      ...lifetimeFields,
    };
    if (!keepFrozenFunding && totals) {
      next.funding = totals.funding;
      next.netPnl = Number(next.closeSlippage || 0) + totals.funding - Number(next.fees || 0);
      next.sessionStartDay = totals.sessionRows[0]?.day || null;
      next.sessionEndDay = totals.sessionRows[totals.sessionRows.length - 1]?.day || null;
      next.sessionDayCount = totals.sessionRows.length;
      next.sessionFundingApplied = true;
    }
    return attachClosedPairSessionMetrics(next);
  }

  if (!totals) return { ...pair, dailyPerformanceSeries: filteredSeries, ...lifetimeFields };
