...
function pairDaysOpen(base, venueA, venueB, paymentSources) {
  const venues = new Set([venueA, venueB]);
  let earliest = null;
  for (const [venue, payments] of Object.entries(paymentSources)) {
    if (!venues.has(venue)) continue;
    for (const p of payments) {
      if (toBaseSymbol(p.symbol) !== base) continue;
      if (earliest == null || p.time < earliest) earliest = p.time;
    }
  }
  if (earliest == null) return null;
  return Math.max((Date.now() - earliest) / 86400000, 1);
}

function attachPairFundingMeta(pair, base, venueA, venueB, paymentSources) {
  pair.fundingByRange = {
  ...
  pair.daysOpen = pairDaysOpen(base, venueA, venueB, paymentSources);
  pair.venueA = venueA;
  pair.venueB = venueB;
}
...
function buildPairedAnalysis({
  ...
  const fundingMaps = { hl: fundingHl, nado: fundingNado, grvt: fundingGrvt, extended: fundingExtended };
  ...
  function pushHlNadoPair(base, hl, na) {
    ...
    attachPairFundingMeta(paired[paired.length - 1], base, 'hyperliquid', 'nado', paymentSources);
  ...
  function pushCrossPair(base, pairType, pairLabel, legA, legB, venueA, venueB, fundingA, fundingB, feesA, feesB, spreadKey) {
    ...
    attachPairFundingMeta(paired[paired.length - 1], base, venueA, venueB, paymentSources);
  ...
  function getFundingSinceOpen(venue, leg, base) {
    ...
    if (venue === 'extended') return fundingExtended[base] ?? 0;
    return 0;
  }
...
async function fetchPerpsDashboard(wallets) {
  ...
  const dailyFundingSeries = buildDailyFundingSeries({
