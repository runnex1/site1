/**
 * Perps DEX helpers — Hyperliquid + Nado + GRVT + Extended + Phoenix.
...
function grvtStateNeedsFallback(state) {
  return Boolean(state?.error) || !(state?.positions || []).length;
}
...
async function resolveGrvtStateWithFallback(subAccountId, liveState, overrideRaw) {
  if (!grvtStateNeedsFallback(liveState) && liveState?.positions?.length) {
  ...
  if (grvtStateNeedsFallback(resolved)) {
    ...
    if (cache) resolved = applyGrvtStateFallback(resolved, cache, 'server-cache');
  }
...
async function fetchPerpsEquitySnapshot(wallets) {
  ...
  const phoenixEnabled = isSolanaAddress(phoenixWallet);
  const [hl, nado, grvt, extended, phoenix] = await Promise.all([
  ...
  const states = [hl, nado, grvt, extended, phoenix];
  ...
  const total = hlAccountValue + nadoAccountValue + grvtAccountValue + extendedAccountValue + phoenixAccountValue;
...
function computeCombinedNetDeposits(hlCapitalFlows, nadoCapitalFlows, grvtCapitalFlows = null, extendedCapitalFlows = null, phoenixCapitalFlows = null, windowMs = CROSS_VENUE_WINDOW_MS) {
  ...
  const rawCombinedNetDeposits = hlNetDeposits + nadoNetDeposits + grvtNetDeposits + extendedNetDeposits + phoenixNetDeposits;
...
function netDepositsAtTime(hlPayments, nadoPayments, timeMs, grvtPayments = null, extendedPayments = null, phoenixPayments = null, windowMs = CROSS_VENUE_WINDOW_MS) {
...
function buildEquitySeries({
  ...
  for (const bf of backfill) {
    ...
    const totalEquity = bf.totalEquity ?? ((hl ?? 0) + (nado ?? 0) + grvt + extended + phoenix);
  ...
  for (const snap of snapshots) {
    ...
    const totalEquity = snap.totalEquity ?? hl + nado + grvt + extended + phoenix;
  ...
  const totalNow = hlNow + nadoNow + grvtNow + extendedNow + phoenixNow;
  points.push({
    time: fetchedAt,
...
function buildDailyFundingSeries({
  ...
  const endDay = isoDateFromMs(seriesEndMs);
  const startDay = isoDateFromMs(seriesEndMs - days * 86400000);
  const points = [];
  let cumFunding = 0;
  let cumFees = 0;
  let cumNet = 0;
...
function buildFundingCumulativeSeries(hlPayments, nadoPayments, days, pairedBases = null, grvtPayments = null, extendedPayments = null, phoenixPayments = null) {
...
/** Solo (never-hedged) Closed rows are only considered inside this lookback. */
const CLOSED_NEVER_HEDGED_LOOKBACK_MS = 14 * 86400000;
...
function closedVenueShortLabel(venue) {
  return ({
    hyperliquid: 'HL',
    nado: 'Nado',
    grvt: 'GRVT',
...
function pushClosedPair(pairs, a, b) {
  ...
  const closeSlippage = closedPairCloseSlippage({ longLeg, shortLeg, size: Math.min(left.size || 0, right.size || 0) });
  ...
  pair.avgNotional = closedPairAvgNotional(pair);
  pair.sessionDays = closedPairSessionDays(pair);
  pair.sessionApr = closedPairSessionApr(pair);
  pairs.push(pair);
}

/** Unhedged-until-close: single exchange leg with no hedge partner. */
function pushExchangeOnlyClosedPair(pairs, leg) {
  if (!leg?.symbol || !leg?.venue || !leg?.closeTime) return;
  const size = Math.abs(Number(leg.size || 0));
  if (!(size > CLOSED_ROUND_EPS)) return;
  const realized = Number(leg.realizedPnl);
  const funding = Number(leg.funding || 0);
  const fees = Math.abs(Number(leg.fees || 0));
  const closeSlippage = Number.isFinite(realized) ? realized : 0;
  const longLeg = leg.side === 'long' ? leg : null;
  const shortLeg = leg.side === 'short' ? leg : null;
  if (!longLeg && !shortLeg) return;
  const pair = {
    symbol: leg.symbol,
    pairLabel: closedVenueShortLabel(leg.venue),
    pairType: 'exchange_only',
    exchangeOnly: true,
    openTime: leg.openTimeKnown ? Number(leg.openTime || 0) || null : null,
    closeTime: Number(leg.closeTime),
    size,
    sizeMismatchPct: 0,
    longLeg,
    shortLeg,
    ...
    netPnl: closeSlippage + (Number.isFinite(funding) ? funding : 0) - (Number.isFinite(fees) ? fees : 0),
  };
  pair.avgNotional = closedPairAvgNotional(pair);
  pair.sessionDays = closedPairSessionDays(pair);
  pair.sessionApr = closedPairSessionApr(pair);
...
function closedPairLegMarginPx(leg) {
  return firstNumber(
    leg?.avgEntryPx,
    leg?.entryPx,
    leg?.avgClosePx,
...
function closedPairAvgNotional(pair) {
  const fallbackSize = Math.abs(Number(pair?.size || 0));
  const legs = [pair?.longLeg, pair?.shortLeg].filter(Boolean);
  const notionals = legs.map((leg) => {
    const px = closedPairLegMarginPx(leg);
    const legSize = Math.abs(Number(leg?.size || fallbackSize));
    return px != null && legSize > 0 ? legSize * px : null;
  }).filter(n => n != null && n > 0);
  if (!notionals.length) return null;
  return notionals.reduce((sum, n) => sum + n, 0) / notionals.length;
}

function closedPairLifetimeMetrics(pair) {
  const closeTime = Number(pair?.closeTime || 0);
  const longLeg = pair?.longLeg;
  const shortLeg = pair?.shortLeg;
  if (!closeTime || !longLeg || !shortLeg) return null;
  if (!longLeg.openTimeKnown || !shortLeg.openTimeKnown) return null;
  const openTime = Math.min(Number(longLeg.openTime || 0), Number(shortLeg.openTime || 0));
  if (!openTime || closeTime <= openTime) return null;
  const funding = Number(pair.funding);
  const fees = Number(pair.fees ?? 0);
  if (!Number.isFinite(funding) || !Number.isFinite(fees)) return null;
  return {
    openTime,
    closeTime,
    days: Math.max((closeTime - openTime) / 86400000, 1 / 24),
    funding,
    fees,
  };
}

function closedPairCarryBasis(pair) {
  const lifetimeDays = Number(pair?.lifetimeDays);
  const lifetimeFunding = Number(pair?.lifetimeFunding);
  const lifetimeFees = Number(pair?.lifetimeFees);
  if (lifetimeDays > 0 && Number.isFinite(lifetimeFunding) && Number.isFinite(lifetimeFees)) {
    return {
      funding: lifetimeFunding,
      fees: lifetimeFees,
      days: lifetimeDays,
      source: 'lifetime',
    };
  ...
  return {
    funding: Number(pair?.funding),
    fees: Number(pair?.fees ?? 0),
    days: closedPairSessionDays(pair),
    source: 'session',
  };
}

function closedPairSessionDays(pair) {
  const lifetimeDays = Number(pair?.lifetimeDays);
  if (lifetimeDays > 0) return lifetimeDays;
  const start = pair?.sessionStartDay;
  const end = pair?.sessionEndDay;
  if (start && end) {
  ...
  const count = Number(pair?.sessionDayCount || 0);
  if (count > 0) return count;
  const openTime = Number(pair?.openTime || 0);
  const closeTime = Number(pair?.closeTime || 0);
  if (openTime > 0 && closeTime > openTime) {
    return (closeTime - openTime) / 86400000;
  }
  return null;
}

function closedPairSessionApr(pair) {
  if (pair?.aprUnavailable || pair?.closeLegEstimated) return null;
  const margin = Number(pair?.avgNotional) > 0 ? Number(pair.avgNotional) : closedPairAvgNotional(pair);
  const carry = closedPairCarryBasis(pair);
  const days = carry.days;
  const netFunding = carry.funding - carry.fees;
  if (!margin || !days || days <= 0 || !Number.isFinite(netFunding)) return null;
  return (netFunding / margin) * (365 / days) * 100;
}

function attachClosedPairSessionMetrics(pair) {
  const next = { ...pair };
  next.avgNotional = closedPairAvgNotional(next) ?? next.avgNotional ?? null;
  next.sessionDays = closedPairSessionDays(next);
  next.sessionApr = closedPairSessionApr(next);
  return next;
}
...
/**
 * Never-hedged classification: full fill round, recent lookback, no opposite-venue
 * activity overlapping the position. Staggered hedges must NOT become SOLO rows.
 */
function closedLegHadOppositeVenueActivity(leg, symbolLegs) {
  const open = Number(leg?.openTime || 0);
  const close = Number(leg?.closeTime || 0);
  if (!close) return false;
  return (symbolLegs || []).some((other) => {
    if (!other || other === leg) return false;
    if (other.venue === leg.venue || other.side === leg.side) return false;
    if (leg.openTimeKnown && other.openTimeKnown && closedLegOpenWindowsOverlap(leg, other)) {
      return true;
    }
    ...
    if (
      otherClose
      && open
      && otherClose >= open - CLOSED_PAIR_SYNTHETIC_MATCH_WINDOW_MS
      && otherClose <= close + CLOSED_PAIR_SYNTHETIC_MATCH_WINDOW_MS
    ) {
      return true;
    }
    if (other.openTimeKnown && open && close) {
      const otherOpen = Number(other.openTime || 0);
      if (otherOpen >= open && otherOpen <= close) return true;
    }
    // Staggered hedges often close hours/days apart — still not never-hedged.
    if (
      otherClose
      && Math.abs(otherClose - close) <= CLOSED_NEVER_HEDGED_LOOKBACK_MS
    ) {
      return true;
    }
    return false;
...
function closedLegIsNeverHedgedSolo(leg, symbolLegs, nowMs = Date.now()) {
  if (!leg?.symbol || !leg?.venue || !leg?.closeTime) return false;
  if (!leg.openTimeKnown || leg.reconstructedFromClosingFills || leg.fromExchangeHistory) return false;
  if (nowMs - Number(leg.closeTime) > CLOSED_NEVER_HEDGED_LOOKBACK_MS) return false;
  if (!(Math.abs(Number(leg.size || 0)) > CLOSED_ROUND_EPS)) return false;
  if (!Number.isFinite(Number(leg.realizedPnl))) return false;
  if (closedLegHadOppositeVenueActivity(leg, symbolLegs)) return false;
  return true;
}
...
function buildClosedPairsFromLegs(legs) {
  ...
  for (const symbolLegs of Object.values(bySymbol)) {
    ...
    symbolLegs.forEach((leg, idx) => {
      ...
      unmatched.push(leg);
      // Only carefully classified never-hedged full rounds inside the 14d lookback become SOLO.
      // Client also demotes synthetic Var cards and drops Var-covered false SOLOs.
      if (closedLegIsNeverHedgedSolo(leg, symbolLegs)) {
        pushExchangeOnlyClosedPair(pairs, leg);
      }
    });
  }
  return {
...
function buildPairDailyPerformanceSeries(dailySeriesInputs, symbol, perfDays, endMs = Date.now()) {
  return buildDailyFundingSeries({
    hlPayments: dailySeriesInputs.hlPayments || [],
    nadoPayments: dailySeriesInputs.nadoPayments || [],
    grvtPayments: dailySeriesInputs.grvtPayments || [],
    extendedPayments: dailySeriesInputs.extendedPayments || [],
    phoenixPayments: dailySeriesInputs.phoenixPayments || [],
    hlFills: dailySeriesInputs.hlFills || [],
    nadoMatches: dailySeriesInputs.nadoMatches || [],
    grvtFills: dailySeriesInputs.grvtFills || [],
    extendedFills: dailySeriesInputs.extendedFills || [],
    phoenixFills: dailySeriesInputs.phoenixFills || [],
    days: perfDays,
    endMs,
    pairedBases: [symbol],
  });
...
function enrichClosedPairSessionPnl(pair, dailySeriesInputs, fillHistoryDays = PERPS_MAX_FILL_HISTORY_DAYS) {
  ...
  const fillSources = {
    hyperliquid: dailySeriesInputs.hlFills || [],
    nado: dailySeriesInputs.nadoMatches || [],
    grvt: dailySeriesInputs.grvtFills || [],
    extended: dailySeriesInputs.extendedFills || [],
    phoenix: dailySeriesInputs.phoenixFills || [],
  };
  const paymentSources = {
    hyperliquid: dailySeriesInputs.hlPayments || [],
    nado: dailySeriesInputs.nadoPayments || [],
    grvt: dailySeriesInputs.grvtPayments || [],
    extended: dailySeriesInputs.extendedPayments || [],
    phoenix: dailySeriesInputs.phoenixPayments || [],
  };

  const lifetime = closedPairLifetimeMetrics(pair);
  const openHint = Number(
    pair.openTime
    || pair.longLeg?.openTime
    || pair.shortLeg?.openTime
    || 0,
  );
  const openDays = openHint > 0
    ? Math.ceil((closeTime - openHint) / 86400000) + 2
    : fillHistoryDays;
  const perfDays = Math.min(PERPS_MAX_FILL_HISTORY_DAYS, Math.max(fillHistoryDays, openDays));
  const dailyPerformanceSeries = buildPairDailyPerformanceSeries(
    dailySeriesInputs,
    symbol,
    perfDays,
    closeTime,
  );
  const session = latestActivitySessionBounds(dailyPerformanceSeries, closeTime);
  const lookbackStartMs = session?.sessionStartMs ?? null;
  const peakPair = applyPeakToCloseMetrics(pair, fillSources, paymentSources, {
    fundingForClosedLeg,
    closeSlippageForPair: closedPairCloseSlippage,
    ...(lookbackStartMs != null ? {
      lookbackStartMs,
      lookbackMs: Math.max(closeTime - lookbackStartMs, 60 * 1000),
    } : {}),
  });

  const windowStart = Number(peakPair.statsSinceMs || peakPair.openTime || lookbackStartMs || 0);
  const sessionSeries = session?.sessionRows?.length
    ? session.sessionRows
    : trimDailySeriesToLatestSession(dailyPerformanceSeries);
  const sessionTotals = pairLatestSessionTotals(sessionSeries.length ? sessionSeries : dailyPerformanceSeries);

  const lifetimeFields = lifetime ? {
    lifetimeFunding: lifetime.funding,
    lifetimeFees: lifetime.fees,
    lifetimeDays: lifetime.days,
    lifetimeOpenTime: lifetime.openTime,
  } : {};

  // Variational closes freeze funding only when a real session total was stored — not hollow $0 wipes.
  const frozenFunding = Number(pair.funding);
  const keepFrozenFunding = Boolean(pair?.manualVariationalClose)
    && Number.isFinite(frozenFunding)
    && !(pair.closeLegEstimated
      && Math.abs(frozenFunding) < 1e-9
      && Math.abs(Number(pair.fees || 0)) < 1e-9);

  if (peakPair.peakMetricsApplied) {
    // Size / fees / slippage: peak→close. Funding: full latest activity session
    // (same total Position Performance showed before close).
    const sessionFunding = sessionTotals && Number.isFinite(Number(sessionTotals.funding))
      ? Number(sessionTotals.funding)
      : null;
    const next = {
      ...peakPair,
      dailyPerformanceSeries: sessionSeries,
      ...lifetimeFields,
      sessionStartDay: session?.sessionRows?.[0]?.day || peakPair.sessionStartDay || null,
      sessionEndDay: session?.sessionRows?.[session.sessionRows.length - 1]?.day
        || peakPair.sessionEndDay
        || null,
      sessionDayCount: session?.sessionRows?.length || peakPair.sessionDayCount || null,
    };
    if (keepFrozenFunding) {
      next.funding = Number(pair.funding);
    } else if (sessionFunding != null) {
      next.funding = sessionFunding;
      next.sessionFundingApplied = true;
      next.fundingSinceMs = lookbackStartMs || next.fundingSinceMs || windowStart || null;
    }
    next.netPnl = Number(next.closeSlippage || 0) + Number(next.funding || 0) - Number(next.fees || 0);
    return attachClosedPairSessionMetrics(next);
  }

  const totals = sessionTotals || pairLatestSessionTotals(dailyPerformanceSeries);
  if (!totals) return { ...pair, dailyPerformanceSeries: sessionSeries, ...lifetimeFields };

  const closeSlippage = Number(pair.closeSlippage || 0);
  const { funding, fees, sessionRows } = totals;

  return attachClosedPairSessionMetrics({
    ...pair,
    dailyPerformanceSeries: sessionRows,
    ...lifetimeFields,
    funding: keepFrozenFunding ? Number(pair.funding) : funding,
    fees: keepFrozenFunding ? Number(pair.fees ?? 0) : fees,
    netPnl: keepFrozenFunding
      ? Number(pair.netPnl ?? (closeSlippage + Number(pair.funding) - Number(pair.fees ?? 0)))
      : closeSlippage + funding - fees,
    ...
    sessionFundingApplied: !keepFrozenFunding,
  });
}
...
function closedPairStableKey(pair) {
  ...
  if (pair?.exchangeOnly || (!longVenue && !shortVenue) || (longVenue && !shortVenue) || (!longVenue && shortVenue)) {
    const solo = pair?.longLeg || pair?.shortLeg;
    const venue = solo?.venue || longVenue || shortVenue;
    const side = solo?.side || (longVenue ? 'long' : 'short');
    if (!venue) return '';
    return `${symbol}|${closeTime}|${venue}|solo|${side}`;
  }
  if (!longVenue || !shortVenue) return '';
...
/** Closed tab / cache retention — only rounds closed within this window. */
const CLOSED_PAIRS_MAX_AGE_MS = 30 * 86400000;

function pruneClosedPairsByAge(pairs, now = Date.now(), maxAgeMs = CLOSED_PAIRS_MAX_AGE_MS) {
  const cutoff = Number(now) - Number(maxAgeMs);
  return (pairs || []).filter((p) => {
    const closeTime = Number(p?.closeTime || p?.closedAt || 0);
    return Number.isFinite(closeTime) && closeTime >= cutoff;
  });
}
...
function buildPairedAnalysis({
  ...
  const closedPairsFiltered = pruneClosedPairsByAge(filterFullyClosedPairs(closedResult.pairs, {
  ...
  const closedPairs = enrichClosedPairsSessionPnl(freshClosedPairs, dailySeriesInputs, fillHistoryDays);
  const knownClosed = new Set(parseKnownClosedKeys(knownClosedKeys));
  const closedPairRefreshes = knownClosed.size
    ? enrichClosedPairsSessionPnl(
      closedPairsFiltered.filter(p => knownClosed.has(closedPairStableKey(p))),
      dailySeriesInputs,
      fillHistoryDays,
    )
    : [];
  ...
  const OPEN_PAIR_MAX_SIZE_MISMATCH_PCT = 25;
  ...
  const totalFees = hlFills.totalFees + nadoMatches.totalFees + (grvtFills?.totalFees || 0) + (extendedFills?.totalFees || 0) + (phoenixFills?.totalFees || 0);
  const totalRealized = hlFills.totalRealized + nadoMatches.totalRealized + (grvtFills?.totalRealized || 0) + (extendedFills?.totalRealized || 0) + (phoenixFills?.totalRealized || 0);
  const totalEntrySlippage = paired.reduce((s, p) => s + (p.entrySlippage || 0), 0);
  const netFunding = hlFunding.totalFunding + nadoFunding.totalFunding + (grvtFunding?.totalFunding || 0) + (extendedFunding?.totalFunding || 0) + (phoenixFunding?.totalFunding || 0);
...
function buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase = {}, phoenixRateByBase = {}) {
  const spreadRows = [];
...
async function fetchPerpsLiveRates(opts = {}) {
  ...
  return {
    fetchedAt: Date.now(),
    rateSpread: buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase, phoenixRateByBase),
  };
}
...
async function fetchPerpsDashboard(wallets) {
  ...
  const [grvtRates, extendedRates, variationalRates, phoenixRates] = await Promise.all([
    grvtEnabled
  ...
  const spreadRows = buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase, phoenixRateByBase);
  ...
  for (const p of arb.paired) {
    const openDays = p.pairOpenedAtMs
      ...
      : fillHistoryDays;
    const perfDays = Math.min(PERPS_MAX_FILL_HISTORY_DAYS, Math.max(fillHistoryDays, openDays));
    p.dailyPerformanceSeries = buildPairDailyPerformanceSeries(dailySeriesInputs, p.symbol, perfDays);
  }
  ...
  const stateFetchedAts = [hlState, nadoState, grvtEnabled ? grvtState : null, extendedEnabled ? extendedState : null, phoenixEnabled ? phoenixState : null]
  ...
  const capitalFlows = {
    ...
    ...computeCombinedNetDeposits(hlCapitalFlows, nadoCapitalFlows, grvtCapitalFlows, extendedCapitalFlows, phoenixCapitalFlows),
  };
  ...
  const equityNow = {
    ...
    total: hlState.accountValue + (nadoState.accountValue ?? 0) + grvtEquity + extendedEquity + phoenixEquity,
    adjustedTotal: hlState.accountValue + (nadoState.accountValue ?? 0) + grvtEquity + extendedEquity + phoenixEquity - capitalFlows.combinedNetDeposits,
  };
  ...
  const dashboard = {
    fetchedAt,
    days,
    wallets: { hyperliquid: hlWallet, nado: nadoWallet, grvtSubAccount, phoenix: phoenixWallet || null },
    grvt: {
      state: grvtState,
      funding: grvtFunding,
      fills: grvtFills,
    ...
    closedPairRefreshes: arb.closedPairRefreshes,
    ...
    summary: {
      ...
      netFundingTotal: hlFunding.totalFunding + nadoFunding.totalFunding + grvtFunding.totalFunding + extendedFundingWindow.totalFunding + phoenixFunding.totalFunding,
      ...
      netFundingTotalAllAccounts: hlFunding.totalFunding + nadoFunding.totalFunding + grvtFunding.totalFunding + extendedFundingWindow.totalFunding + phoenixFunding.totalFunding,
  ...
  return slimPerpsDashboardForClient(dashboard);
}

/** Drop heavy fields that freeze the browser on JSON.parse + dashboard render. */
function slimPerpsDashboardForClient(payload) {
  ...
  for (const p of payload.closedPairs || []) {
    if (p?.symbol) activeSymbols.add(String(p.symbol).toUpperCase());
  }
  ...
  return {
    ...
    closedDebug: undefined,
    paired: Array.isArray(payload.paired) ? payload.paired.map(slimPair) : payload.paired,
    closedPairs: Array.isArray(payload.closedPairs) ? payload.closedPairs.map(slimPair) : payload.closedPairs,
...
function buildEquitySnapshotFromDashboard(data) {
  ...
  const totalEquity = hlAccountValue + nadoAccountValue + grvtAccountValue + extendedAccountValue + phoenixAccountValue;
  ...
  const adj = data?.variationalEquityAdjust || {};
  const openAdj = Number(adj.openAdj);
  const pendingAdj = Number(adj.pendingAdj);
  const closedAdj = Number(adj.closedAdj);
  const crossVenueAdj = Number(adj.crossVenueAdj);
  const totalAdj = Number.isFinite(Number(adj.totalAdj))
    ? Number(adj.totalAdj)
    : [openAdj, pendingAdj, closedAdj, crossVenueAdj].filter(Number.isFinite).reduce((sum, value) => sum + value, 0);
  const record = {
    date,
    bucket: key,
    hlAccountValue,
  ...
  if (Number.isFinite(totalAdj)) {
    record.variationalEquityAdjust = totalAdj;
    if (Number.isFinite(openAdj)) record.variationalOpenEquityAdjust = openAdj;
    if (Number.isFinite(pendingAdj)) record.variationalPendingCloseEquityAdjust = pendingAdj;
    if (Number.isFinite(closedAdj)) record.variationalClosedEquityAdjust = closedAdj;
    if (Number.isFinite(crossVenueAdj)) record.crossVenueSameMarkAdjust = crossVenueAdj;
  }
  return {
    key,
    record,
...
function appendEquitySnapshotStore(store, data, maxEntries = 180) {
  ...
  if (next[key]) {
    const prev = next[key];
    const prevAdj = Number(prev.variationalEquityAdjust);
    const nextAdj = Number(record.variationalEquityAdjust);
    const partsChanged = [
      ...
      const b = Number(record[k]);
      if (!Number.isFinite(a) && !Number.isFinite(b)) return false;
      if (!Number.isFinite(a) || !Number.isFinite(b)) return true;
      return Math.abs(a - b) > 0.01;
    });
    const totalChanged = Number.isFinite(prevAdj) && Number.isFinite(nextAdj)
      ? Math.abs(prevAdj - nextAdj) > 0.01
      : Number.isFinite(nextAdj);
    if (Number.isFinite(prevAdj) && !partsChanged && !totalChanged) return next;
    next[key] = {
      ...prev,
      ...record,
      variationalEquityAdjust: record.variationalEquityAdjust,
      variationalOpenEquityAdjust: record.variationalOpenEquityAdjust,
      variationalPendingCloseEquityAdjust: record.variationalPendingCloseEquityAdjust,
      variationalClosedEquityAdjust: record.variationalClosedEquityAdjust,
      crossVenueSameMarkAdjust: record.crossVenueSameMarkAdjust,
    };
  } else {
    next[key] = record;
  }
...
async function fetchPerpsCapitalFlowsBundle(wallets) {
  ...
  const computed = computeCombinedNetDeposits(hl, nado, grvt, extended, phoenix);
  return {
    hl,
    nado,
    grvt,
...
async function fetchPerpsEquitySnapshotWithVariational(wallets, opts = {}) {
  ...
  const closedPairs = opts.closedPairs || [];
  const grvtSubAccount = wallets.grvtSubAccount
    || process.env.GRVT_SUB_ACCOUNT_ID
    || DEFAULT_GRVT_SUB_ACCOUNT;
  const grvtEnabled = Boolean(grvtSubAccount && process.env.GRVT_API_KEY);
  ...
  const [equityBase, hlState, nadoState, grvtState, extendedState, capitalFlows] = await Promise.all([
    fetchPerpsEquitySnapshot(wallets),
    fetchHyperliquidState(hlWallet),
    fetchNadoState(nadoWallet).catch(() => ({ positions: [] })),
    grvtEnabled
      ? fetchGrvtState(grvtSubAccount).catch(() => ({ positions: [] }))
      : Promise.resolve({ positions: [] }),
    extendedEnabled
      ? fetchExtendedState().catch(() => ({ positions: [] }))
      : Promise.resolve({ positions: [] }),
    refreshCapitalFlows
  ...
  const { computeVariationalEquityAdjustFromHedges } = require('./variational-equity');
  const variationalEquityAdjust = computeVariationalEquityAdjustFromHedges({
    hedges,
    closedPairs,
    states: {
      hyperliquid: { state: hlState },
      nado: { state: nadoState },
      grvt: { state: grvtState },
      extended: { state: extendedState },
      phoenix: { state: { positions: [] } },
    },
  });
...
module.exports = {
  ...
  closedPairLifetimeMetrics,
  closedPairCarryBasis,
  closedPairSessionDays,
  closedPairSessionApr,
  attachClosedPairSessionMetrics,
  filterFreshClosedPairs,
  pruneClosedPairsByAge,
  CLOSED_PAIRS_MAX_AGE_MS,
