...
async function fetchPerpsDashboard(wallets) {
  ...
  const [
    ...
    withTimeout(fetchHyperliquidFunding(hlWallet, days), PERPS_CORE_FETCH_TIMEOUT_MS, 'Hyperliquid funding'),
    withTimeout(fetchNadoFunding(nadoWallet, days, 'default', activeNadoSymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO funding'),
    ...
    grvtEnabled ? withTimeout(fetchGrvtFunding(grvtSubAccount, days), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'GRVT funding').catch(e => ({
