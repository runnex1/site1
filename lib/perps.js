...
function mapGrvtFillRow(row) {
  ...
  return {
    ...
    side: (row.is_buyer ?? row.ib) ? 'buy' : 'sell',
...
async function grvtPaginate(path, baseBody, windowStartMs, opts = {}) {
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    if (windowStartMs != null) {
      ...
      body.end_time = msToGrvtNs(Date.now());
    }
...
async function fetchGrvtFills(subAccountId, days = 30) {
  ...
  const rows = await grvtPaginate('fill_history', baseBody, windowStart);
...
async function fetchGrvtPositionHistory(subAccountId, days = 30) {
  ...
  for (let page = 0; page < PERPS_GRVT_HISTORY_MAX_PAGES; page++) {
    const data = await grvtTradesPost('position_history', {
      ...baseBody,
      start_time: msToGrvtNs(windowStart),
      end_time: msToGrvtNs(Date.now()),
      limit: 500,
      cursor,
    });
    const batch = grvtBatch(data);
...
function extendedClosedTimeMs(row) {
  const t = Number(row.closedTime || 0);
  if (!t) return null;
...
async function fetchExtendedPositionHistory(days = 30) {
  ...
  const windowStart = Date.now() - days * 86400000;
  const rows = await extendedPaginate('/user/positions/history', { limit: 500 }, windowStart);
  const positions = rows.filter(row => {
...
async function fetchNadoMatches(wallet, days = 30, subaccountName = 'default', symbols = null) {
  ...
  async function fetchMatchChunk(chunk) {
    let maxIdx = undefined;
    for (let page = 0; page < 30; page++) {
      const body = {
        matches: {
  ...
  for (let i = 0; i < productIds.length; i += 32) {
    await fetchMatchChunk(productIds.slice(i, i + 32));
  }
...
const PERPS_HISTORY_DAYS = 30;
const PERPS_MAX_FILL_HISTORY_DAYS = PERPS_HISTORY_DAYS;
...
function buildDailyFundingSeries({
  ...
  const addFunding = (payments, venue) => {
    for (const p of payments) {
      ...
      fundingEventsByDay[day].push({ time: p.time, usdc: p.usdc || 0, venue });
    }
  };
...
function buildClosedLegsForVenue(venue, fills, paymentSources) {
  ...
  for (const [symbol, rows] of Object.entries(bySymbol)) {
    ...
    for (const fill of sorted) {
      const delta = fillSignedSize(fill);
      if (!delta) continue;
      if (!round.length && Math.abs(Number(fill.closedPnl ?? fill.realizedPnl ?? 0)) > CLOSED_ROUND_EPS) {
        // The first visible fill is probably closing a position opened before our history window.
        continue;
      }
...
function grvtPositionIsClosed(row) {
  ...
  if (typeof status === 'number') return status >= 1 && status <= 3;
...
function buildClosedLegsFromExchangeHistory(exchangeHistory, paymentSources) {
  ...
  for (const row of grvtHistory) {
    ...
    const isLong = row.is_long ?? row.il;
    legs.push({
      venue: 'grvt',
      symbol,
  ...
  for (const row of extendedHistory) {
    const closeTime = extendedClosedTimeMs(row);
    const openTime = Number(row.createdTime || 0);
    ...
    legs.push({
      ...
      symbol,
      side: sideRaw === 'SHORT' ? 'short' : 'long',
      size: parseFloat(row.maxPositionSize || row.size || 0),
      openTime,
      closeTime,
      openTimeKnown: true,
...
function pushClosedPair(pairs, a, b) {
  ...
  const closeSlippage = a.realizedPnl + b.realizedPnl;
  const funding = a.funding + b.funding;
  const fees = a.fees + b.fees;
  pairs.push({
    symbol: a.symbol,
    pairLabel: `${a.venue} + ${b.venue}`,
    openTime: a.openTimeKnown && b.openTimeKnown ? Math.min(a.openTime, b.openTime) : null,
    closeTime: Math.max(a.closeTime, b.closeTime),
    size: Math.min(a.size || 0, b.size || 0),
    sizeMismatchPct: Math.max(a.size || 0, b.size || 0) > 0
      ? Math.abs((a.size || 0) - (b.size || 0)) / Math.max(a.size || 0, b.size || 0) * 100
      : 0,
    longLeg,
    shortLeg,
    closeSlippage,
...
function mergeVenueClosedLegs(historyLegs, fillLegs) {
  ...
  for (const fillLeg of fillLegs) {
    if (merged.some(h => closedLegsOverlap(h, fillLeg))) continue;
    merged.push(fillLeg);
  }
  return merged;
}

function buildClosedPairs(fillSources, paymentSources, exchangeHistory = null) {
  const useExtendedHistory = Boolean(exchangeHistory?.extended?.length);
  ...
  const extendedFillLegs = useExtendedHistory
    ? []
    : buildClosedLegsForVenue('extended', fillSources.extended || [], paymentSources);
    ...
    while (true) {
      ...
      for (let i = 0; i < symbolLegs.length; i++) {
        ...
        for (let j = 0; j < symbolLegs.length; j++) {
          ...
          if (closeGap > CLOSED_PAIR_MATCH_WINDOW_MS) continue;
          const openGap = a.openTimeKnown && b.openTimeKnown ? Math.abs(a.openTime - b.openTime) : 0;
          const score = closeGap + openGap * 0.25;
          if (score < bestScore) {
            bestScore = score;
            bestI = i;
            bestJ = j;
  ...
  return pairs.sort((a, b) => b.closeTime - a.closeTime);
}
...
function buildPairedAnalysis({
  ...
  spreadRows,
  days,
  fillHistoryDays = PERPS_HISTORY_DAYS,
}) {
  ...
  const closedPairs = buildClosedPairs(fillSources, paymentSources, {
...
async function fetchPerpsDashboard(wallets) {
  ...
  const historyDays = PERPS_HISTORY_DAYS;
  ...
  const [hlState, nadoState] = await Promise.all([
    ...
    withTimeout(fetchNadoState(nadoWallet), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO state'),
  ]);
  ...
  const [
    ...
    withTimeout(fetchHyperliquidFunding(hlWallet, historyDays), PERPS_CORE_FETCH_TIMEOUT_MS, 'Hyperliquid funding'),
    withTimeout(fetchNadoFunding(nadoWallet, historyDays, 'default', activeNadoSymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO funding').catch(e => ({
      venue: 'nado', wallet: nadoWallet, days: historyDays, payments: [], totalFunding: 0, error: errorMessage(e),
    })),
    withTimeout(fetchHyperliquidFills(hlWallet, historyDays), PERPS_CORE_FETCH_TIMEOUT_MS, 'Hyperliquid fills'),
    withTimeout(fetchNadoMatches(nadoWallet, historyDays, 'default', activeNadoSymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO matches').catch(e => ({
      venue: 'nado', wallet: nadoWallet, days: historyDays, matches: [], totalFees: 0, totalRealized: 0, error: errorMessage(e),
    })),
    withTimeout(fetchHyperliquidRates(), PERPS_CORE_FETCH_TIMEOUT_MS, 'Hyperliquid rates'),
    withTimeout(fetchNadoRates(), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO rates'),
    ...
    grvtEnabled ? withTimeout(fetchGrvtFunding(grvtSubAccount, historyDays), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'GRVT funding').catch(e => ({
      venue: 'grvt', subAccountId: grvtSubAccount, days: historyDays, payments: [], totalFunding: 0, error: errorMessage(e),
    })) : Promise.resolve({ venue: 'grvt', subAccountId: grvtSubAccount, days: historyDays, payments: [], totalFunding: 0 }),
    grvtEnabled ? withTimeout(fetchGrvtFills(grvtSubAccount, historyDays), PERPS_GRVT_HISTORY_TIMEOUT_MS, 'GRVT fills').catch(e => ({
      venue: 'grvt', subAccountId: grvtSubAccount, days: historyDays, fills: [], totalFees: 0, totalRealized: 0, rawRowCount: 0, error: errorMessage(e),
    })) : Promise.resolve({ venue: 'grvt', subAccountId: grvtSubAccount, days: historyDays, fills: [], totalFees: 0, totalRealized: 0, rawRowCount: 0 }),
    grvtEnabled ? withTimeout(fetchGrvtPositionHistory(grvtSubAccount, historyDays), PERPS_GRVT_HISTORY_TIMEOUT_MS, 'GRVT position history').catch(e => ({
      venue: 'grvt', subAccountId: grvtSubAccount, days: historyDays, positions: [], rawRowCount: 0, error: errorMessage(e),
    })) : Promise.resolve({ venue: 'grvt', subAccountId: grvtSubAccount, days: historyDays, positions: [], rawRowCount: 0 }),
    ...
    extendedEnabled ? withTimeout(fetchExtendedFunding(historyDays), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Extended funding').catch(e => ({
      venue: 'extended', days: historyDays, payments: [], totalFunding: 0, error: errorMessage(e),
    })) : Promise.resolve({ venue: 'extended', days: historyDays, payments: [], totalFunding: 0 }),
    extendedEnabled ? withTimeout(fetchExtendedFills(historyDays), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Extended fills').catch(e => ({
      venue: 'extended', days: historyDays, fills: [], totalFees: 0, totalRealized: 0, error: errorMessage(e),
    })) : Promise.resolve({ venue: 'extended', days: historyDays, fills: [], totalFees: 0, totalRealized: 0 }),
    extendedEnabled ? withTimeout(fetchExtendedPositionHistory(historyDays), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Extended position history').catch(e => ({
      venue: 'extended', days: historyDays, positions: [], error: errorMessage(e),
    })) : Promise.resolve({ venue: 'extended', days: historyDays, positions: [] }),
  ...
  const needsNadoClosedHistory = nadoHistorySymbols.length > activeNadoBaseSet.size
    || nadoHistorySymbols.some(symbol => !activeNadoBaseSet.has(symbol));
  if (needsNadoClosedHistory) {
    const [nf, nm] = await Promise.all([
      withTimeout(fetchNadoFunding(nadoWallet, historyDays, 'default', nadoHistorySymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO funding history').catch(e => ({
        ...nadoFunding,
        error: errorMessage(e),
      })),
      withTimeout(fetchNadoMatches(nadoWallet, historyDays, 'default', nadoHistorySymbols), PERPS_CORE_FETCH_TIMEOUT_MS, 'NADO match history').catch(e => ({
        ...nadoMatches,
        error: errorMessage(e),
      })),
    ]);
    nadoFundingForAnalysis = nf;
    nadoMatchesForAnalysis = nm;
  }
  ...
  for (const p of arb.paired) {
    const openDays = p.pairOpenedAtMs
      ...
      : historyDays;
    const perfDays = Math.min(PERPS_HISTORY_DAYS, Math.max(historyDays, openDays));
  ...
  return {
    ...
    summary: {
      ...
      nadoError: combineErrors(nadoCapitalFlows),
      grvtAccountValue: grvtEquity,
      extendedAccountValue: extendedEquity,
