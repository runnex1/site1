...
async function fetchNadoState(wallet, subaccountName = 'default') {
  ...
  const tpslResult = await fetchNadoTriggerOrders(subaccount, positions);
  attachTpslToPositions(positions, tpslResult.map);
  ...
  if (tpslResult.error) errors.push(`NADO TP/SL unavailable: ${tpslResult.error}`);
  return {
    venue: 'nado',
    wallet,
    subaccount,
