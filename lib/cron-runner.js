...
const { fetchLoopRates } = require('./loop-rates');
const {
...
function nextDueFrom(lastSuccess, spec, now = Date.now()) {
  ...
  if (Number.isFinite(spec.minute)) {
    if (!last) {
      ...
      if (current.getUTCMinutes() === spec.minute) return now;
      return thisHour.getTime() <= now ? nextHourlyMinute(now, spec.minute) : thisHour.getTime();
    }
...
function isDue(job, state, now = Date.now()) {
  ...
  if (!lastSuccess) {
    if (Number.isFinite(spec.minute)) return new Date(now).getUTCMinutes() === spec.minute;
    return true;
  }
  if (Number.isFinite(spec.minute)) {
    const due = nextDueFrom(lastSuccess, spec, now);
    return now >= due && new Date(now).getUTCMinutes() === spec.minute;
  }
  return now - lastSuccess >= spec.everyMs;
}
...
async function runLoopsSync() {
  ...
  await kvSet(CACHE_KEYS.loopRates, JSON.stringify({
    key: wallets.map(w => w.toLowerCase()).sort().join(','),
    fetchedAt: Date.now(),
    data,
  }));
  return {
...
async function runDueJobs({ maxJobs = 2 } = {}) {
  ...
  const due = Object.keys(JOBS).filter(job => isDue(job, states[job], now));
...
async function getCronStatus() {
  ...
  for (const [job, spec] of Object.entries(JOBS)) {
    ...
    const nextDue = nextDueFrom(state?.lastSuccess, spec, now);
    jobs[job] = {
