...
const { applyPeakToCloseMetrics } = _positionPeakWindow || {};
const {
...
function venueTrackedLegFetchUncertain(data, venue) {
  const stateKey = VENUE_STATE_KEY[venue];
...
function applyVariationalPeakToClosePair(pair, hedge, listing, data) {
  ...
  const peakSize = Math.abs(Number(next.size || trackedLeg.size || 0));
  ...
  const signedVar = resolveVariationalSignedSize(hedge, {
    size: trackedLeg.side === 'short' ? -peakSize : peakSize,
    side: trackedLeg.side,
  });

  trackedLeg.size = peakSize;
  varLeg.size = peakSize;
  const hedgeForFunding = {
    ...hedge,
    status: 'closed',
    closedAt: closeTime,
    variationalSize: signedVar,
    trackedSize: peakSize,
  };
  varLeg.funding = buildVariationalFundingEventsScheduled(hedgeForFunding, listing, {
    sinceMs: windowStart,
    now: closeTime,
    variationalSize: signedVar,
  }).reduce((sum, ev) => sum + (ev.usdc || 0), 0);
  if (isPositivePx(varLeg.avgEntryPx) && isPositivePx(varLeg.avgClosePx)) {
    varLeg.realizedPnl = variationalLegPnl(signedVar, varLeg.avgEntryPx, varLeg.avgClosePx);
  }

  const longPnl = Number.isFinite(Number(next.longLeg?.realizedPnl)) ? Number(next.longLeg.realizedPnl) : 0;
  const shortPnl = Number.isFinite(Number(next.shortLeg?.realizedPnl)) ? Number(next.shortLeg.realizedPnl) : 0;
  next.closeSlippage = longPnl + shortPnl;
  next.funding = Number(next.longLeg?.funding ?? 0) + Number(next.shortLeg?.funding ?? 0);
...
function buildVariationalClosedPair(hedge, trackedCloseLeg, listing, data = null) {
  ...
  const trackedLeg = {
    ...
    funding: Number(closeLeg.funding ?? 0),
    ...
    closeTime: Number(closeLeg.closeTime || hedge.closedAt || Date.now()),
    closeLegEstimated,
  };
  ...
  const peaked = data ? applyVariationalPeakToClosePair(pair, hedge, listing, data) : pair;
