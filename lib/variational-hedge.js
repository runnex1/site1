...
const VARIATIONAL_RATE_SAMPLE_LIMIT = 2000;
 ...
 * Prefer stored Variational samples near T−10s; live listing only when freshly observed;
 * optional reference-exchange history (Bybit/Binance) as last resort for past intervals.
 */
function resolveVariationalFreezeMarketInputs({
  ...
  const refMap = referenceRateByTime && typeof referenceRateByTime === 'object'
    ? referenceRateByTime
    : null;
  if (refMap) {
    const settlementTime = sampleAt + VARIATIONAL_SETTLEMENT_SAMPLE_LEAD_MS;
    let ref = refMap[settlementTime] || refMap[String(settlementTime)] || null;
    if (!ref) {
      // Binance occasionally offsets fundingTime by 1ms; accept nearest within 2s.
      let bestKey = null;
      let bestDist = Infinity;
      for (const key of Object.keys(refMap)) {
        const t = Number(key);
        if (!Number.isFinite(t)) continue;
        const dist = Math.abs(t - settlementTime);
        if (dist <= 2000 && dist < bestDist) {
          bestDist = dist;
          bestKey = key;
        }
      }
      if (bestKey != null) ref = refMap[bestKey];
    }
    const rate = Number(ref?.rate ?? ref);
    const markPx = Number(ref?.markPx) || variationalLiveMarkPx(listing) || null;
    if (Number.isFinite(rate) && markPx) {
      return {
        markPx,
        rate,
        intervalS: Number(ref?.intervalS) || normalizeVariationalListing(listing)?.fundingIntervalS || FUNDING_INTERVAL_8H_S,
        freezeSource: 'reference-history',
        rateSource: ref?.source || 'reference-history',
        observedAtMs: Number(ref?.atMs) || settlementTime,
      };
    }
  }
...
function captureVariationalSettlementsDue(hedge, listing, storedSettlements = [], opts = {}) {
  ...
  const referenceRateByTime = opts.referenceRateByTime || null;
...
function buildVariationalFundingEventsFrozen(hedge, listing, storedSettlements = [], opts = {}) {
  ...
  for (const t of settlementTimes) {
    ...
    const market = resolveVariationalFreezeMarketInputs({
      ...
      referenceRateByTime: opts.referenceRateByTime || null,
    });
    if (market) {
...
function resolveVariationalCloseSlippagePnl(data, hedge, trackedLeg, closeTime, fallbackSize, hlClosePx) {
  if (computeLegPeakWindow && data && hedge) {
    ...
    const peak = computeLegPeakWindow(fills, symbol, closeTime);
...
function finalizeVariationalCloseIfReady(hedge, data, listing, findCloseLeg = findTrackedCloseLeg) {
  if (findTrackedLeg(data, hedge)) return null;
