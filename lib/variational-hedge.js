...
function accumulatePartialCloseRealizedPnl(hedge, data) {
  ...
  const since = Number(hedge._lastPartialCloseCheckMs || hedge.openedAt || 0);
  const fills = (dashboardFillSources(data)[venue] || [])
    .filter((f) => toBaseSymbol(f.symbol) === symbol)
    .filter((f) => !since || Number(f.time || 0) >= since - 60000)
    .filter((f) => Number(f.sz ?? f.size ?? 0) > 0 && fillIsClosingSide(f, positionSide))
    .sort((a, b) => Number(a.time || 0) - Number(b.time || 0));
  if (!fills.length) {
    hedge._lastTrackedSize = liveTracked;
    hedge._lastPartialCloseCheckMs = Date.now();
    return hedge;
  }

  const hasExchangePnl = fills.some(
    (f) => Math.abs(Number(f.closedPnl ?? f.realizedPnl ?? 0)) > (CLOSED_ROUND_EPS || 1e-8),
  );
  let realized = 0;
  if (hasExchangePnl) {
