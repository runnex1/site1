...
/** Pair Variational estimates to tracked-exchange funding timestamps when both legs share a pair. */
function buildVariationalFundingEventsAligned(hedge, listing, trackedEvents, opts = {}) {
  const size = Number(hedge?.variationalSize ?? -Number(hedge?.trackedSize));
  const entryPx = Number(hedge?.variationalEntryPx);
  if (!hedge || !size || !Number.isFinite(entryPx)) return [];

  const rate = listing?.fundingRateInterval ?? 0;
  const markPx = listing?.markPx || entryPx;
  const avgPx = (entryPx + markPx) / 2;
  const aligned = (trackedEvents || []).map((te) => ({
    venue: 'variational',
    time: te.time,
    usdc: variationalFundingPaymentPerInterval(size, avgPx, rate),
    symbol: hedge.symbol,
    intervalHours: te.intervalHours ?? listing?.fundingIntervalHours ?? 8,
    fundingEstimated: true,
  }));
  if (aligned.length) return aligned;
  return buildVariationalFundingEvents(hedge, listing, opts);
}
...
function estimateVariationalFundingUsd(hedge, listing, now = Date.now()) {
  ...
  const size = Number(hedge?.variationalSize ?? -Number(hedge?.trackedSize));
  const openedAt = Number(hedge?.openedAt);
  if (!openedAt || !Number.isFinite(entryPx) || !size) return 0;

  const intervalS = listing?.fundingIntervalS || FUNDING_INTERVAL_8H_S;
  const rate = listing?.fundingRateInterval ?? 0;
  const markPx = listing?.markPx || entryPx;
  const endAt = hedge?.status === 'closed'
    ? Number(hedge?.closedAt || now)
    : now;
  const intervalMs = intervalS * 1000;
  const intervals = Math.max(0, Math.floor((endAt - openedAt) / intervalMs));
  const avgPx = (entryPx + (markPx || entryPx)) / 2;
  return intervals * variationalFundingPaymentPerInterval(size, avgPx, rate);
}
...
function variationalMarkPx(listing, hedge) {
  const live = Number(listing?.markPx);
  if (Number.isFinite(live) && live > 0) return live;
  const stored = Number(hedge?.variationalMarkAtOpen);
  if (Number.isFinite(stored) && stored > 0) return stored;
  return null;
}

/**
 * Entry reference for open uPnL: mark at hedge open (excludes size slippage on fill).
 * Falls back to stored fill when mark-at-open was not captured (legacy hedges).
 */
function variationalUpnlEntryPx(hedge) {
  const markAtOpen = Number(hedge?.variationalMarkAtOpen);
  if (Number.isFinite(markAtOpen) && markAtOpen > 0) return markAtOpen;
  const fill = Number(hedge?.variationalEntryPx);
  return Number.isFinite(fill) && fill > 0 ? fill : null;
}

/** One-time entry slippage vs Variational mark at open (fill worse than mark on large size). */
function variationalEntrySlippageUsd(size, fillPx, markAtOpenPx) {
  const fill = Number(fillPx);
  const mark = Number(markAtOpenPx);
  if (!size || !Number.isFinite(fill) || !Number.isFinite(mark) || fill <= 0 || mark <= 0) return null;
  return variationalLegPnl(size, fill, mark);
}

function buildVariationalSyntheticLeg(hedge, listing) {
  const size = Number(hedge?.variationalSize ?? -Number(hedge?.trackedSize));
  const fillPx = Number(hedge?.variationalEntryPx);
  const markPx = variationalMarkPx(listing, hedge) || (Number.isFinite(fillPx) && fillPx > 0 ? fillPx : null);
  const upnlEntryPx = variationalUpnlEntryPx(hedge);
  const markAtOpen = Number(hedge?.variationalMarkAtOpen);
  const hasFill = Number.isFinite(fillPx) && fillPx > 0;
  const hasUpnl = upnlEntryPx != null && markPx != null;
  const notional = Math.abs(size) * (markPx || fillPx || 0);
  const funding = estimateVariationalFundingUsd(hedge, listing);
  const entrySlippageUsd = hasFill && Number.isFinite(markAtOpen) && markAtOpen > 0
    ? variationalEntrySlippageUsd(size, fillPx, markAtOpen)
    : null;
  return {
    venue: 'variational',
    size,
    side: size > 0 ? 'long' : 'short',
    entryPx: hasFill ? fillPx : null,
    fillPx: hasFill ? fillPx : null,
    markAtOpenPx: Number.isFinite(markAtOpen) && markAtOpen > 0 ? markAtOpen : null,
    markPx,
    notional,
    unrealizedPnl: hasUpnl ? variationalLegPnl(size, upnlEntryPx, markPx) : null,
    entrySlippageUsd,
...
function buildVariationalOpenPair(trackedLeg, hedge, listing, spread) {
  ...
  const entrySlippage = varLeg.entrySlippageUsd ?? null;
  const combinedUpnl = varLeg.unrealizedPnl == null
    ? null
    : (trackedLeg.unrealizedPnl ?? 0) + varLeg.unrealizedPnl + (entrySlippage ?? 0);
...
function createHedgeFromUnhedged(unhedgedLeg, entryPx, markAtOpen = null) {
  const trackedSize = Number(unhedgedLeg.size);
  const varEntry = Number(entryPx);
  const openMark = Number(markAtOpen);
  return {
    ...
    variationalMarkAtOpen: Number.isFinite(openMark) && openMark > 0 ? openMark : null,
...
const variationalHedgeExports = {
  ...
  variationalLegPnl,
  variationalMarkPx,
  variationalUpnlEntryPx,
  variationalEntrySlippageUsd,
  netFundingSpread8h,
