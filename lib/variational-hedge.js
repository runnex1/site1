...
function variationalClosedPairCoversHedgeSession(pair, hedge, data = null) {
  ...
  if (pair.variationalHedgeId && hedge.id && pair.variationalHedgeId === hedge.id) return true;
  const hist = data
...
function buildVariationalSyntheticLeg(hedge, listing, trackedLeg = null) {
  ...
  if (trackedLeg != null && Number.isFinite(Number(trackedLeg.unrealizedPnl))) {
  ...
  return {
    ...
    fundingEstimated: true,
    variationalUpnlOffset: trackedLeg != null && Number.isFinite(Number(trackedLeg.unrealizedPnl)),
  };
}
...
function finalizeVariationalCloseIfReady(hedge, data, listing, findCloseLeg = findTrackedCloseLeg, opts = {}) {
  ...
  const trackedCloseLeg = enrichTrackedCloseLegPx(data, hedge, findCloseLeg(data, hedge, listing));
  ...
  if (!exitPx) {
    ...
    exitPx = deriveVariationalExitPx(resolvePendingCloseMarkPx(hedge, listing, data), signedVar);
  }
...
function computeVariationalClosedPairFunding(
  ...
  const historyOpen = trackedCloseLegRef?.fromExchangeHistory
    ? Number(trackedCloseLegRef.openTime || 0)
    : 0;
  // GRVT/Extended history rows cover the full exchange session; Var funding still starts at hedge open.
  const trackedFundingStart = historyOpen > 0
    ? historyOpen
    : hedgeOpenedAt;
  ...
  if (!hasTrackedPaymentEvidence) {
    ...
    && Number.isFinite(Number(trackedCloseLegRef?.funding))
    && Math.abs(Number(tracked) - Number(trackedCloseLegRef.funding)) > 1
  ) {
    // Prefer full-session history cashflow when payment-window sum diverges (late hedge attach).
    tracked = Number(trackedCloseLegRef.funding);
  }
  const hedgeForFunding = {
    ...hedge,
    status: 'closed',
...
function freezeVariationalClosedFunding(hedge, listing, data, closeTime, signedVar, trackedCloseLegRef = null) {
  ...
  const breakdown = computeVariationalClosedPairFunding(
    ...
    closeTime,
    signedVar,
    // Prefer exchange history / close leg so mid-session attaches freeze full-session funding.
...
function applyVariationalHedges(data, hedges, variationalBySymbol, opts = {}) {
  ...
  const closedPairAlreadyCached = (hedge) => (data.closedPairs || []).some((p) => (
    ...
    || variationalClosedPairCoversHedgeSession(p, hedge, data)
  ));
