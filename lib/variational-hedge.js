...
/** Closed Variational leg PnL = adverse 0.12% vs HL close only (HL leg carries hedged price move). */
...
function applyVariationalPeakToClosePair(pair, hedge, listing, data, trackedCloseLegRef = null) {
  ...
  const hlClosePx = trackedCloseLegExitPx(trackedLeg, data, hedge)
    ?? trackedCloseLegExitPx(trackedCloseLegRef, data, hedge);
  const derivedExit = deriveVariationalExitPx(hlClosePx, signedVar);
  const entryPx = Number(hedge?.variationalEntryPx ?? varLeg.avgEntryPx);
  ...
  if (isPositivePx(hlClosePx) && isPositivePx(varLeg.avgClosePx)) {
    varLeg.realizedPnl = variationalLegCloseSlippagePnl(signedVar, hlClosePx, varLeg.avgClosePx);
  }
...
function buildVariationalClosedPair(hedge, trackedCloseLeg, listing, data = null) {
  ...
  const varRealized = hlClosePx && exitPx
