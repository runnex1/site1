...
function finalizeVariationalCloseIfReady(hedge, data, listing, findCloseLeg = findTrackedCloseLeg, opts = {}) {
  ...
  if (!exitPx) {
    ...
    const listingMark = Number(listing?.markPx ?? listing?.mark);
    exitPx = deriveVariationalExitPx(listingMark, signedVar);
  }
...
function findTrackedCloseLeg(data, hedge, listing = null) {
  ...
  if (fallback) {
    ...
    if (estimated != null && Number.isFinite(estimated)) {
      const closePx = trackedCloseLegExitPx(fallback, data, hedge)
        || (isPositivePx(listingMark) ? listingMark : null)
  ...
  const snap = hedge.trackedLastSnapshot;
  if (!snap) return null;
  const snapClosePx = Number(snap.markPx)
    || (isPositivePx(listingMark) ? listingMark : null);
  return {
