...
function parseVariationalListing(listing) {
  ...
  // Variational API returns funding_rate as percent per interval (e.g. 0.1095 = 0.1095%), not decimal.
  const fundingRatePct = parseFloat(listing.funding_rate);
  const fundingIntervalS = parseInt(listing.funding_interval_s, 10) || FUNDING_INTERVAL_8H_S;
  const fundingRateInterval = Number.isFinite(fundingRatePct) ? fundingRatePct / 100 : null;
  const fundingRate8h = Number.isFinite(fundingRateInterval)
    ? fundingRateInterval * (FUNDING_INTERVAL_8H_S / fundingIntervalS)
    : null;
  ...
  return {
    ...
    fundingIntervalS,
    fundingRate8h,
    fundingIntervalHours: fundingIntervalS / 3600,
  };
}
...
function buildVariationalSyntheticLeg(hedge, listing) {
  ...
  return {
    ...
    notional,
    unrealizedPnl: hasEntry ? variationalLegPnl(size, entryPx, markPx || entryPx) : null,
...
function normalizeTrackedLeg(leg, venue, symbol) {
  ...
  return {
    ...
    fundingSinceOpen: leg.fundingSinceOpen ?? leg.funding ?? 0,
    fees: leg.fees ?? 0,
...
function buildVariationalOpenPair(trackedLeg, hedge, listing, spread) {
  ...
  const combinedUpnl = (trackedLeg.unrealizedPnl ?? 0) + (varLeg.unrealizedPnl ?? 0);
...
function findTrackedCloseLeg(data, hedge) {
  ...
  return {
    ...
    realizedPnl: Number(snap.unrealizedPnl ?? 0),
...
function createHedgeFromUnhedged(unhedgedLeg, entryPx) {
  ...
  return {
    ...
    variationalEntryPx: Number(entryPx),
