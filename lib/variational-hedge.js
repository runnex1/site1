...
/**
 * Rebuild a hollow estimated Variational close so empty-fill peak metrics cannot stick forever.
 * Safe no-op when exchange fill evidence already produced real tracked metrics.
 */
function repairHollowVariationalClosedPair(pair, hedge, listing, data = null) {
  if (!variationalClosedPairLooksHollow(pair) || !hedge) return pair;
  ...
  if (!rebuilt || variationalClosedPairLooksHollow(rebuilt)) return pair;
  return {
    ...pair,
    ...rebuilt,
...
function applyVariationalPeakToClosePair(pair, hedge, listing, data, trackedCloseLegRef = null) {
  ...
  if (!hasTrackedFillPnl && Number.isFinite(Number(prePeakTracked?.realizedPnl))) {
    trackedLeg.realizedPnl = Number(prePeakTracked.realizedPnl);
  }
  if (!hasTrackedFeeFills && Number.isFinite(Number(prePeakTracked?.fees))) {
    trackedLeg.fees = Number(prePeakTracked.fees);
  }
  ...
  const peakTrackedRealized = hasTrackedFillPnl && Number.isFinite(Number(trackedLeg.realizedPnl))
...
function computeVariationalClosedPairFunding(
  ...
  const venue = hedge?.trackedVenue;
  const symbol = hedge?.symbol;
  let tracked = venue && fundingForClosedLeg
  ...
  const hasTrackedPaymentEvidence = payments.some((p) => {
    ...
    if (t < fundingStart || t > end) return false;
  ...
  if (!hasTrackedPaymentEvidence) {
    ...
    if (Number.isFinite(snapFunding)) tracked = snapFunding;
  }
  const hedgeForFunding = {
    ...hedge,
    status: 'closed',
