...
function resolveVariationalSizesOnEntryEdit(hedge, trackedLeg, opts = {}) {
  if (!hedge) return hedge;
  const explicitVarSize = Number(opts.variationalSize);
  ...
  if (Number.isFinite(explicitVarSize) && explicitVarSize !== 0) {
    hedge.variationalSize = liveTracked > 0 ? -Math.abs(explicitVarSize) : Math.abs(explicitVarSize);
    return hedge;
  }

  if (Number.isFinite(liveTracked) && liveTracked !== 0) {
    const trackedMoved = Number.isFinite(storedTracked) && storedTracked !== 0
      && Math.abs(liveTracked - storedTracked) / Math.max(Math.abs(storedTracked), Math.abs(liveTracked), 1) > 0.0001;
    if (!Number.isFinite(storedVar) || storedVar === 0 || trackedMoved) {
      hedge.variationalSize = -Math.abs(liveTracked);
    } else {
      hedge.variationalSize = storedVar;
    }
  } else if (Number.isFinite(storedTracked) && storedTracked !== 0 && !Number.isFinite(storedVar)) {
