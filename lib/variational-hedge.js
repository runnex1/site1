...
/** Prefer tracked exchange mark so hedge legs share the same price reference. */
function resolveHedgeMarkPx(trackedLeg, listing) {
  const trackedMark = Number(trackedLeg?.markPx);
  if (Number.isFinite(trackedMark) && trackedMark > 0) return trackedMark;
  const listingMark = Number(listing?.markPx);
  if (Number.isFinite(listingMark) && listingMark > 0) return listingMark;
  const entry = Number(trackedLeg?.entryPx);
  return Number.isFinite(entry) && entry > 0 ? entry : null;
}

function hedgeLegSizeRatio(trackedLeg, varLeg) {
  const trackedAbs = Math.abs(Number(trackedLeg?.size || 0));
  const varAbs = Math.abs(Number(varLeg?.size || 0));
  if (!trackedAbs || !varAbs) return null;
  return varAbs / trackedAbs;
}

function isMatchedOppositeHedge(trackedLeg, varLeg, tolerance = 0.0001) {
  const trackedSize = Number(trackedLeg?.size || 0);
  const varSize = Number(varLeg?.size || 0);
  if (!trackedSize || !varSize) return false;
  if (Math.sign(trackedSize) === Math.sign(varSize)) return false;
  const ratio = hedgeLegSizeRatio(trackedLeg, varLeg);
  if (ratio == null) return false;
  return Math.abs(ratio - 1) <= tolerance;
}

/** Mirror tracked exchange uPnL for matched opposite legs (delta-neutral display). */
function mirrorVariationalUpnl(trackedLeg, varLeg) {
  if (!isMatchedOppositeHedge(trackedLeg, varLeg)) return null;
  const trackedUpnl = Number(trackedLeg?.unrealizedPnl);
  if (!Number.isFinite(trackedUpnl)) return null;
  const ratio = hedgeLegSizeRatio(trackedLeg, varLeg);
  return ratio == null ? null : -trackedUpnl * ratio;
}

function buildVariationalSyntheticLeg(hedge, listing, trackedLeg = null) {
  const size = Number(hedge?.variationalSize ?? -Number(hedge?.trackedSize));
  const entryPx = Number(hedge?.variationalEntryPx);
  const markPx = resolveHedgeMarkPx(trackedLeg, listing) || listing?.markPx || entryPx;
  ...
  const formulaUpnl = hasEntry ? variationalLegPnl(size, entryPx, markPx || entryPx) : null;
  const mirroredUpnl = trackedLeg ? mirrorVariationalUpnl(trackedLeg, { size }) : null;
  return {
    venue: 'variational',
    size,
    side: size > 0 ? 'long' : 'short',
    entryPx: hasEntry ? entryPx : null,
    markPx: markPx || entryPx,
    notional,
    unrealizedPnl: mirroredUpnl != null ? mirroredUpnl : formulaUpnl,
    unrealizedPnlRaw: mirroredUpnl != null ? formulaUpnl : null,
    hedgeUpnlMirrored: mirroredUpnl != null,
...
function createHedgeFromUnhedged(unhedgedLeg, entryPx) {
  ...
  const trackedEntry = Number(unhedgedLeg.entryPx ?? unhedgedLeg.entry);
  ...
  return {
    ...
    variationalEntryPx: Number.isFinite(varEntry) && varEntry > 0
      ? varEntry
      : (Number.isFinite(trackedEntry) && trackedEntry > 0 ? trackedEntry : null),
...
const variationalHedgeExports = {
  ...
  variationalLegPnl,
  resolveHedgeMarkPx,
  mirrorVariationalUpnl,
  isMatchedOppositeHedge,
  netFundingSpread8h,
