 ...
 * Strictly time-ordered and idempotent with price feed continuity across rebalances.
 */
...
function normalizeTokensForPnl(tokens) {
  return [...tokensToMap(tokens).values()]
...
class FifoQtyLedger {
  ...
  avgCost() {
    const totalQ = this.qty();
    if (totalQ <= EPS) return 0;
    const totalC = this.lots.reduce((s, l) => s + (l.qty * l.costPerUnit), 0);
  ...
  sellQty(qty, pricePerUnit) {
    ...
    const rawPrice = num(pricePerUnit, 0);
    const price = rawPrice > EPS ? rawPrice : this.avgCost();
...
function processQtyTransition(ledger, prevAmt, prevVal, nextAmt, nextVal, fallbackPrice = 0) {
  ...
  const effectivePrice = nextPrice > EPS
    ? nextPrice
    : (prevPrice > EPS ? prevPrice : (fallbackPrice > EPS ? fallbackPrice : ledger.avgCost()));

  if (pAmt <= EPS && nAmt > EPS) {
    const cost = nVal > EPS ? nVal : (effectivePrice > EPS ? nAmt * effectivePrice : 0);
    ledger.addLot(nAmt, cost);
    return;
  }
  if (pAmt > EPS && nAmt <= EPS) {
    const sellPx = prevPrice > EPS ? prevPrice : (effectivePrice > EPS ? effectivePrice : ledger.avgCost());
    ledger.sellQty(pAmt, sellPx);
    return;
  }
  if (nAmt + EPS < pAmt) {
    const sellPx = nextPrice > EPS ? nextPrice : (prevPrice > EPS ? prevPrice : effectivePrice);
    ledger.sellQty(pAmt - nAmt, sellPx);
  } else if (nAmt > pAmt + EPS) {
    const addQty = nAmt - pAmt;
    const addPx = nextPrice > EPS ? nextPrice : (prevPrice > EPS ? prevPrice : effectivePrice);
    ledger.addLot(addQty, addQty * addPx);
  }
}
...
function snapshotFingerprint(kind, snap) {
  if (kind === 'wallet') {
    return JSON.stringify(normalizeTokensForPnl(snap.tokens || []));
  }
...
/** Strictly time-ordered and idempotent deduplication of PnL snapshots */
function dedupePnlSnapshots(snapshots) {
  const valid = (snapshots || [])
    .filter(s => s && Number.isFinite(num(s.ts, NaN)))
    .map(s => {
      const ts = num(s.ts);
      const kind = s.tokens ? 'wallet' : 'protocol';
      const fp = snapshotFingerprint(kind, s);
      return { snap: { ...s, ts }, ts, kind, fp };
    });

  // Strict deterministic ordering: ts ascending, then fingerprint tie-breaker
  valid.sort((a, b) => {
  ...
  for (const item of valid) {
    // If exact same timestamp, prefer the most recent/complete entry
    if (lastTs !== null && item.ts === lastTs) {
      if (item.fp !== lastFp && out.length) {
        out[out.length - 1] = item.snap;
        lastFp = item.fp;
      }
      continue;
    }
    // If consecutive identical state, skip redundant point
    if (item.fp === lastFp && out.length) continue;
...
function aggregateWalletTotal(trackers, tokenMap, priceCache) {
  let total = 0;
  const seen = new Set();
  for (const [sym, t] of tokenMap) {
    ...
    if (!ledger) continue;
    const rawPrice = num(t.amount, 0) > EPS ? num(t.value, 0) / num(t.amount, 0) : 0;
    const price = rawPrice > EPS ? rawPrice : (priceCache.get(sym) || ledger.avgCost());
...
function computeWalletPnlSeries(snapshots) {
  ...
  for (const snap of series) {
    ...
    // Update price cache for all tokens with active prices
    for (const [sym, t] of tokenMap) {
      if (t.amount > EPS && t.value > EPS) {
        priceCache.set(sym, t.value / t.amount);
      }
    }
    ...
    for (const sym of keys) {
      ...
      const cachedPrice = priceCache.get(sym) || 0;
    ...
    const totalPnl = aggregateWalletTotal(trackers, tokenMap, priceCache);
