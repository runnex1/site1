/**
 * KV store using Upstash Redis REST API directly.
 * Works with the env vars automatically set when you connect
 * Upstash to your Vercel project:
 *   KV_REST_API_URL + KV_REST_API_TOKEN
 * or
 *   REDIS_URL (Upstash REST URL)
 */

function getUrl()   { return process.env.KV_REST_API_URL  || process.env.REDIS_URL || ''; }
function getToken() { return process.env.KV_REST_API_TOKEN || process.env.KV_REST_API_READ_ONLY_TOKEN || ''; }

async function upstashRequest(command) {
  const url   = getUrl();
  const token = getToken();

  if (!url || !token) {
    // Fallback to in-memory if no KV configured
    return null;
  }

  const res = await fetch(`${url}/${command.map(encodeURIComponent).join('/')}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error(`Upstash error: ${res.status}`);
  const json = await res.json();
  return json.result;
}

async function kvGet(key) {
  try {
    const result = await upstashRequest(['GET', key]);
    return result;
  } catch (e) {
    console.error('[kv] GET error:', e.message);
    return null;
  }
}

async function kvSet(key, value) {
  const payload = typeof value === 'string' ? value : JSON.stringify(value);
  if (!getUrl() || !getToken()) {
    console.error('[kv] SET skipped: no KV configured for key', key);
    throw new Error('KV store is not configured');
  }
  try {
    await upstashRequest(['SET', key, payload]);
    return true;
  } catch (e) {
    console.error('[kv] SET error:', key, e.message);
    throw e;
  }
}

async function kvDel(key) {
  try {
    await upstashRequest(['DEL', key]);
  } catch (e) {
    console.error('[kv] DEL error:', e.message);
  }
}

module.exports = { kvGet, kvSet, kvDel };
