...
function event(kind, text, ts, opts = {}) {
  const defaults = {
    ...
    'Kobeissi Letter': ['news', 'var(--amber)', 'rgba(245,158,11,0.10)'],
...
function shouldReplaceEvent(prev, next) {
  ...
  if (next.kind === 'Kobeissi BREAKING' || next.kind === 'Kobeissi Letter') {
  ...
  for (const handle of ['kobeissiletters', 'KobeissiLetters']) {
    try {
      ...
      for (const block of blocks.slice(-25)) {
        ...
        const isBreaking = /\bBREAKING\b/i.test(text);
        const kind = isBreaking ? 'Kobeissi BREAKING' : 'Kobeissi Letter';
        const key = `${kind}:${headline.toLowerCase()}`;
        if (!byKey.has(key)) {
          byKey.set(key, event(kind, headline, ts, {
            key: `kobeissi:${kind}:${headline.toLowerCase()}`,
            sourceTs: true,
          }));
        }
      }
...
async function buildPolymarketActivity(now) {
  ...
  try {
    ...
    groupTradesByFillWindow(trades || []).slice(0, 8).forEach(g => {
      ...
      items.push(event('Order Filled', `${g.title.slice(0, 70)} - ${g.outcome ? `${g.outcome} ` : ''}${g.side} ${g.totalShares.toFixed(0)} shares @ ${(avg * 100).toFixed(1)}c`, g.latestTs || now, {
