...
async function fetchBreakingPosts() {
  ...
  for (const handle of ['kobeissiletters', 'KobeissiLetters']) {
    try {
      ...
      for (const block of blocks.slice(-30)) {
        ...
        if (!byText.has(headline)) byText.set(headline, event('Kobeissi BREAKING', headline, ts, {
  ...
  return [...byText.values()].sort((a, b) => b.ts - a.ts).slice(0, 10);
}
...
async function collectEvents({ force = false } = {}) {
  ...
  fresh.push(...await fetchBreakingPosts());
