...
async function collectEvents({ force = false } = {}) {
  ...
  const history = parseJson(historyRaw, []).filter(e =>
    e?.kind !== 'Order Filled' && e?.kind !== 'PM Price Move'
  );
...
module.exports = { collectEvents, dateFromText, scanPortfolioEvents, dedupeAndTrim, walletSuffix4, WALLET_SUFFIX_OVERRIDES };
