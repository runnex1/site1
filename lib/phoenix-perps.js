...
function createPhoenixApi({ fetchWithTimeout, withTimeout, errorMessage, toBaseSymbol, timeoutMs }) {
    ...
    for (const sub of subaccounts) {
      for (const row of sub?.positions || []) {
        ...
        const unrealizedPnl = Number.isFinite(markPx) && Number.isFinite(entryPx)
          ? size * (markPx - entryPx)
          : 0;
        const notional = Number.isFinite(markPx)
          ? Math.abs(size * markPx)
          : Math.abs(phoenixQuoteLotsToUsd(row.virtualQuotePositionLots));
        positions.push({
          venue: 'phoenix',
          symbol,
          size,
  ...
  async function fetchPhoenixState(authority) {
    ...
    // Fill missing marks from per-market endpoint for open symbols only.
    const snapshot = stateRes.data?.snapshot || {};
    const openSymbols = [];
    for (const sub of snapshot.subaccounts || []) {
      for (const row of sub.positions || []) {
        const sym = toBaseSymbol(row.symbol);
        if (sym && Math.abs(Number(row.basePositionLots) || 0) > 0 && markBySymbol[sym] == null) {
          openSymbols.push(sym);
        }
      }
    }
    await Promise.all(openSymbols.slice(0, 12).map(async (symbol) => {
      try {
  ...
  async function fetchPhoenixCapitalFlows(authority) {
    ...
    for (const row of rows) {
      ...
      let usdc = 0;
      if (type === 'deposit') usdc = Math.abs(amountUsd);
      else if (type === 'withdrawal' || type === 'withdraw') usdc = -Math.abs(amountUsd);
      else continue;
      payments.push({
        venue: 'phoenix',
        time,
