...
function phoenixPositionUnrealizedPnl({ virtualQuoteLots, size, markPx, entryPx }) {
  const virtualQuote = phoenixQuoteLotsToUsd(virtualQuoteLots);
  if (Number.isFinite(markPx) && Number.isFinite(size) && Number.isFinite(virtualQuote)) {
    return virtualQuote + size * markPx;
  }
