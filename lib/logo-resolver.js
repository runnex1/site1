...
const LOOP_PINNED_TOKEN_LOGOS = {
  USDM: path.join(__dirname, '..', 'public', 'loop-logos', 'usdm.png'),
  JUICED: path.join(__dirname, '..', 'public', 'loop-logos', 'juiced.png'),
};
...
function isLoopPinnedTokenLogo(symbol) {
  return Object.prototype.hasOwnProperty.call(
    LOOP_PINNED_TOKEN_LOGOS,
    String(symbol || '').toUpperCase(),
  );
}

function readLocalLoopLogoDataUrl(symbol) {
  const filePath = LOOP_PINNED_TOKEN_LOGOS[String(symbol || '').toUpperCase()];
