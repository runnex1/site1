...
(function (root, factory) {
  ...
  function applyPeakToCloseMetrics(pair, fillSources, paymentSources, opts = {}) {
    ...
    for (const key of legKeys) {
      ...
      const fees = sumFeesInWindow(fills, symbol, windowStart, closeTime);
      const realizedPnl = sumClosedPnlInWindow(fills, symbol, windowStart, closeTime);
      const funding = fundingInWindow(
        leg.venue,
        symbol,
        windowStart,
        closeTime,
      ...
      exchangeLegs.push({ key, leg, peak, fees, realizedPnl, funding });
    }
    ...
    const windowStart = Math.min(...exchangeLegs.map((row) => row.peak.windowStart));
    ...
    const sessionDays = sessionDaysFromWindow(windowStart, closeTime);
    const startDay = new Date(windowStart).toISOString().slice(0, 10);
    const endDay = new Date(closeTime).toISOString().slice(0, 10);
    const dailyPerformanceSeries = filterDailySeriesSince(pair.dailyPerformanceSeries, windowStart);
