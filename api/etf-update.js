...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!portfolio || !Array.isArray(portfolio.etfs)) return res.status(200).json({ ok: true, updated: 0 });

    const canDca = romaniaHour() >= 12;
    const today = todayRomaniaKey();
    let updated = 0;
    let dcaApplied = 0;
    ...
    for (const e of portfolio.etfs) {
      ...
      if (!ticker) continue;
      let quote = null;
      try { quote = await yahooQuote(ticker); } catch {}
      ...
      e.longName = quote?.longName || e.longName || '';
      e.shortName = quote?.shortName || e.shortName || '';
    ...
    await kvSet('vault:portfolio', JSON.stringify(portfolio));
    return res.status(200).json({ ok: true, updated, dcaApplied, portfolio });
  } catch (e) {
